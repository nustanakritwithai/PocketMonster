...
startupText('กำลังสร้าง Monster Life RPG V7.0.7…');
...
const joyEl=el('joystick'); if(!joyEl) throw new Error('V7.0.7 boot: #joystick not found');
const stick=el('stick'); if(!stick) throw new Error('V7.0.7 boot: #stick not found');
...
// ---------- V7.0.7 Ranch / life / training core ----------
...
function saveGame(show=true){
  ...
  if(show)msg('บันทึกเกม V7.0.7 แล้ว');
}
function loadGame(){
  try{
    ...
    else if(saved.source==='current')msg('โหลดข้อมูล V7.0.7 แล้ว');
    else msg('ย้าย Save เก่า → V7.0.7 สำเร็จ');
