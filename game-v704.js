...
startupText('กำลังสร้าง Monster Life RPG V7.0.5…');
...
const sun=new THREE.DirectionalLight(0xffffff,2.15); sun.position.set(9,18,8); sun.castShadow=true; scene.add(sun);
const ground=new THREE.Mesh(new THREE.PlaneGeometry(90,90),new THREE.MeshStandardMaterial({color:0x59cd61,roughness:1}));
ground.rotation.x=-Math.PI/2; ground.receiveShadow=true; scene.add(ground);

const decorations=new THREE.Group(); scene.add(decorations);
function makeRock(x,z,s=1){ const m=new THREE.Mesh(new THREE.DodecahedronGeometry(s,0),new THREE.MeshStandardMaterial({color:0x945a38,roughness:1})); m.position.set(x,s*.75,z); m.scale.y=1.35; m.castShadow=m.receiveShadow=true; decorations.add(m); }
function makeTree(x,z,s=1){
  const trunk=new THREE.Mesh(new THREE.CylinderGeometry(.22*s,.34*s,1.7*s,7),new THREE.MeshStandardMaterial({color:0x754428})); trunk.position.set(x,.85*s,z); trunk.castShadow=true; decorations.add(trunk);
  const crown=new THREE.Mesh(new THREE.ConeGeometry(1.1*s,2.2*s,8),new THREE.MeshStandardMaterial({color:0x18753a})); crown.position.set(x,2.2*s,z); crown.castShadow=true; decorations.add(crown);
}
...
function makePad(x,z,r,color,opacity=.2){
  const disk=new THREE.Mesh(new THREE.CircleGeometry(r,40),new THREE.MeshBasicMaterial({color,transparent:true,opacity,side:THREE.DoubleSide})); disk.rotation.x=-Math.PI/2; disk.position.set(x,.025,z); scene.add(disk);
  const ring=new THREE.Mesh(new THREE.RingGeometry(r-.12,r,40),new THREE.MeshBasicMaterial({color,transparent:true,opacity:.85,side:THREE.DoubleSide})); ring.rotation.x=-Math.PI/2; ring.position.copy(disk.position); scene.add(ring); return {disk,ring};
}
...
const baseInc=new THREE.Mesh(new THREE.CylinderGeometry(.62,.75,.35,16),new THREE.MeshStandardMaterial({color:0x6d28d9,metalness:.2,roughness:.6})); baseInc.position.y=.18; incubator.add(baseInc);
const eggVisual=new THREE.Mesh(new THREE.SphereGeometry(.34,18,14),new THREE.MeshStandardMaterial({color:0xfde68a,emissive:0x7c2d12,emissiveIntensity:.12})); eggVisual.scale.y=1.28; eggVisual.position.y=.72; incubator.add(eggVisual);
...
function makeInstance(sp,level=1,opts={}){
  ...
  refreshStats(inst,true); return inst;
}
...
function orb(color,r=0.1,seg=10){ return new THREE.Mesh(new THREE.SphereGeometry(r,seg,seg),mat(color,.58,.12)); }
function blackEye(scale=.05){ return new THREE.Mesh(new THREE.SphereGeometry(scale,8,6),new THREE.MeshBasicMaterial({color:0x111827})); }
function addEyeSet(group,{y=1.02,z=-.34,spread=.12,size=.045,browColor=null,browTilt=.15}={}){
  ...
  if(browColor!==null){
    for(const x of [-spread,spread]){ const brow=new THREE.Mesh(new THREE.BoxGeometry(.14,.025,.03),mat(browColor,.6,.1)); brow.position.set(x,y+.09,z-.01); brow.rotation.z=x<0?browTilt:-browTilt; group.add(brow); }
  }
}
function addLegs(group,color,scale=1,count=2,width=.14,height=.38,front=.12){
  ...
  for(let i=0;i<xs.length;i++){
    const leg=new THREE.Mesh(new THREE.CylinderGeometry(width*.62*scale,width*.78*scale,height*scale,7),mat(color,.82,.03));
...
function addTail(group,color,{scale=1,length=.42,thick=.08,pos=[0,.72,.34],rot=[-.9,0,0],tipColor=null}={}){
  const tail=new THREE.Mesh(new THREE.CylinderGeometry(thick*.55*scale,thick*scale,length*scale,8),mat(color,.72,.08));
...
function addEarPair(group,color,{scale=1,height=.28,width=.12,y=1.28,z=0,style='cone',innerColor=null,tilt=.18}={}){
  for(const x of [-.2,.2]){
    let ear;
    if(style==='leaf') ear=new THREE.Mesh(new THREE.ConeGeometry(width*scale,height*scale,6),mat(color,.68,.06));
    else if(style==='bug') ear=new THREE.Mesh(new THREE.CylinderGeometry(width*.18*scale,width*.18*scale,height*scale,5),mat(color,.75,.08));
    else ear=new THREE.Mesh(new THREE.ConeGeometry(width*scale,height*scale,6),mat(color,.68,.06));
    ear.position.set(x*scale,y*scale,z*scale); ear.rotation.z=x<0?tilt:-tilt; ear.castShadow=true; group.add(ear);
    if(innerColor){ const inner=new THREE.Mesh(new THREE.ConeGeometry(width*.55*scale,height*.72*scale,5),mat(innerColor,.75,.02)); inner.position.set(x*scale,(y+.02)*scale,(z-.01)*scale); inner.rotation.z=ear.rotation.z; group.add(inner); }
  }
}
function addWingPair(group,color,{scale=1,y=.9,z=.05,span=.65,lift=.15}={}){
  for(const x of [-1,1]){
    const wing=new THREE.Mesh(new THREE.ConeGeometry(.34*scale,span*scale,3,1,true),mat(color,.74,.04));
...
function addFinPair(group,color,{scale=1,y=.68,z=.05,span=.3,back=.18}={}){
  for(const x of [-1,1]){
    const fin=new THREE.Mesh(new THREE.ConeGeometry(.14*scale,span*scale,3),mat(color,.74,.03));
...
function addHorn(group,color,{x=0,y=1.34,z=-.05,scale=1,length=.32,tilt=0,rx=-.15}={}){
  const horn=new THREE.Mesh(new THREE.ConeGeometry(.08*scale,length*scale,7),mat(color,.55,.24));
...
function addBackSpikes(group,color,{scale=1,count=3,startY=1.0,startZ=.05,gap=.15}={}){
  for(let i=0;i<count;i++){
    const spk=new THREE.Mesh(new THREE.ConeGeometry(.08*scale,.2*scale,5),mat(color,.68,.1));
...
function addShell(group,color,{scale=1}={}){
  const shell=new THREE.Mesh(new THREE.SphereGeometry(.38*scale,14,10,0,Math.PI*2,0,Math.PI*.65),mat(color,.6,.25));
...
function addMuzzle(group,color,{scale=1,y=.95,z=-.42,w=.18,h=.12,d=.14,noseColor=0x1f2937}={}){
  const muz=new THREE.Mesh(new THREE.SphereGeometry(.14*scale,12,10),mat(color,.8,.02));
  muz.scale.set(w/.14,h/.14,d/.14); muz.position.set(0,y*scale,z*scale); muz.castShadow=true; group.add(muz);
  const nose=new THREE.Mesh(new THREE.SphereGeometry(.032*scale,8,6),mat(noseColor,.5,.04)); nose.position.set(0,(y+.01)*scale,(z-.08)*scale); group.add(nose);
  return muz;
}
...
function addWhiskers(group,color,{scale=1,y=.95,z=-.5,len=.16}={}){
  for(const sx of [-1,1]) for(const dy of [-.03,.03]){ const w=new THREE.Mesh(new THREE.CylinderGeometry(.006*scale,.006*scale,len*scale,4),mat(color,.9,0)); w.rotation.z=sx>0?Math.PI/2.6:-Math.PI/2.6; w.position.set(sx*.17*scale,(y+dy)*scale,z*scale); group.add(w);} }
function addCaptureBall(group,{x=.46,y=.95,z=.02,scale=1}={}){
  const ball=new THREE.Group();
  const top=new THREE.Mesh(new THREE.SphereGeometry(.12*scale,14,12,0,Math.PI*2,0,Math.PI/2),mat(0xef4444,.55,.08)); top.position.y=.0; ball.add(top);
  const bot=new THREE.Mesh(new THREE.SphereGeometry(.12*scale,14,12,0,Math.PI*2,Math.PI/2,Math.PI/2),mat(0xf8fafc,.8,.02)); bot.position.y=.0; ball.add(bot);
  const band=new THREE.Mesh(new THREE.CylinderGeometry(.12*scale,.12*scale,.03*scale,18),mat(0x111827,.45,.1)); band.rotation.z=Math.PI/2; ball.add(band);
...
function addShoulderedArm(group,color,{x=.32,y=1.14,z=.03,upper=.24,lower=.2,thick=.06,handColor=null,side=1,shoulderAngle=.28,elbowAngle=.28,forward=.02,holdBall=false}={}){
  ...
  const upperArm=new THREE.Mesh(new THREE.CapsuleGeometry(thick*.5,upper,4,8),mat(color,.78,.02));
  ...
  const foreArm=new THREE.Mesh(new THREE.CapsuleGeometry(thick*.44,lower,4,8),mat(color,.8,.02));
  foreArm.position.set(0,-lower*.34,.01);
  foreArm.castShadow=true;
  elbowPivot.add(foreArm);

  const hand=new THREE.Mesh(new THREE.SphereGeometry(thick*.82,10,8),mat(handColor||color,.74,.02));
...
function makeAnimalBase(color,scale=1,{kind='quadruped',accent=null}={}){
  ...
  let body,head;
  if(kind==='bird'){
    body=new THREE.Mesh(new THREE.SphereGeometry(.42*scale,16,12),mat(color,.72,.08)); body.scale.set(1,1.08,.98); body.position.y=.66*scale;
    head=new THREE.Mesh(new THREE.SphereGeometry(.28*scale,14,12),mat(accent||color,.72,.06)); head.position.set(0,1.08*scale,-.1*scale);
    addLegs(g,color,scale,2,.08,.34,.1);
    addPawSet(g,0xf6ad31,{count:2,scale,r:.05,spreadX:.12,frontZ:.1,y:.03});
  }else if(kind==='serpent'){
    body=new THREE.Mesh(new THREE.CapsuleGeometry(.27*scale,.62*scale,6,12),mat(color,.72,.08)); body.rotation.z=Math.PI/2; body.position.set(0,.63*scale,.02*scale);
    head=new THREE.Mesh(new THREE.SphereGeometry(.3*scale,14,12),mat(accent||color,.72,.06)); head.position.set(0,1.0*scale,-.26*scale);
    addFinPair(g,color,{scale,y:.62,z:.06,span:.28,back:.2});
  }else{
    body=new THREE.Mesh(new THREE.CapsuleGeometry(.26*scale,.5*scale,6,12),mat(color,.72,.07)); body.rotation.z=Math.PI/2; body.position.set(0,.62*scale,.06*scale);
    head=new THREE.Mesh(new THREE.SphereGeometry(.3*scale,14,12),mat(accent||color,.7,.06)); head.position.set(0,1.0*scale,-.18*scale);
...
function makeSlimeMesh(color,scale=1,type='Normal'){
  ...
  const body=new THREE.Mesh(new THREE.SphereGeometry(.46*scale,20,16),jelly(color,.72));
  body.scale.set(1.06,1,.95); body.position.y=.56*scale; body.castShadow=true; body.receiveShadow=true; g.add(body);
  const inner=new THREE.Mesh(new THREE.SphereGeometry(.28*scale,14,12),new THREE.MeshStandardMaterial({color:accentColor,transparent:true,opacity:.28,roughness:.18,metalness:0,emissive:accentColor,emissiveIntensity:.18}));
  inner.position.set(0,.5*scale,.03*scale); inner.scale.set(.9,.78,.85); g.add(inner);
  const base=new THREE.Mesh(new THREE.CircleGeometry(.42*scale,22),new THREE.MeshBasicMaterial({color:0x000000,transparent:true,opacity:.12,side:THREE.DoubleSide}));
  base.rotation.x=-Math.PI/2; base.position.y=.03; g.add(base);
  const shine=new THREE.Mesh(new THREE.SphereGeometry(.16*scale,12,10),new THREE.MeshStandardMaterial({color:0xffffff,transparent:true,opacity:.24,roughness:.08,metalness:0}));
  shine.position.set(-.15*scale,.84*scale,-.17*scale); shine.scale.set(.72,1.12,.5); g.add(shine);
  const sideShine=new THREE.Mesh(new THREE.SphereGeometry(.09*scale,10,8),new THREE.MeshStandardMaterial({color:0xffffff,transparent:true,opacity:.12,roughness:.08,metalness:0}));
  ...
  const mouth=new THREE.Mesh(new THREE.TorusGeometry(.06*scale,.012*scale,6,12,Math.PI),new THREE.MeshBasicMaterial({color:0x1f2937}));
  mouth.rotation.x=Math.PI/2; mouth.position.set(0,.46*scale,-.37*scale); g.add(mouth);
  const nub=new THREE.Mesh(new THREE.SphereGeometry(.08*scale,10,8),jelly(color,.78));
  nub.position.set(0,1.0*scale,.02*scale); nub.scale.set(.8,.7,.8); nub.castShadow=true; g.add(nub);
  const crown=new THREE.Mesh(new THREE.ConeGeometry(.08*scale,.22*scale,6),jelly(accentColor,.84));
  crown.position.set(0,1.16*scale,-.03*scale); crown.castShadow=true; g.add(crown);

  const addOrb=(c,r,x,y,z,op=.8)=>{ const o=new THREE.Mesh(new THREE.SphereGeometry(r*scale,10,8),jelly(c,op)); o.position.set(x*scale,y*scale,z*scale); o.castShadow=true; g.add(o); return o; };
  const addCone=(c,r,h,x,y,z,rx=0,ry=0,rz=0,op=.82)=>{ const o=new THREE.Mesh(new THREE.ConeGeometry(r*scale,h*scale,6),jelly(c,op)); o.position.set(x*scale,y*scale,z*scale); o.rotation.set(rx,ry,rz); o.castShadow=true; g.add(o); return o; };
  const addPlate=(c,w,h,d,x,y,z,rx=0,ry=0,rz=0,op=.84)=>{ const m=new THREE.Mesh(new THREE.BoxGeometry(w*scale,h*scale,d*scale),jelly(c,op)); m.position.set(x*scale,y*scale,z*scale); m.rotation.set(rx,ry,rz); m.castShadow=true; g.add(m); return m; };
  const addRing=(c,r,t,y,z=0,rx=Math.PI/2,op=.65)=>{ const m=new THREE.Mesh(new THREE.TorusGeometry(r*scale,t*scale,8,18),jelly(c,op)); m.position.set(0,y*scale,z*scale); m.rotation.x=rx; g.add(m); return m; };
...
function makeSpeciesMesh(sp,inst=null){
  ...
  switch(path.form){
    ...
    case 'voltkit': {
      ...
      const seg1=new THREE.Mesh(new THREE.BoxGeometry(.08*scale,.22*scale,.05*scale),mat(0xffe45c,.56,.1)); boltTail.add(seg1);
      const seg2=new THREE.Mesh(new THREE.BoxGeometry(.08*scale,.18*scale,.05*scale),mat(0xffe45c,.56,.1)); seg2.position.set(.09*scale,.13*scale,.0); seg2.rotation.z=.7; boltTail.add(seg2);
      const seg3=new THREE.Mesh(new THREE.BoxGeometry(.08*scale,.16*scale,.05*scale),mat(0xffe45c,.56,.1)); seg3.position.set(.0,.24*scale,.0); seg3.rotation.z=-.7; boltTail.add(seg3);
    ...
    case 'mossbun': {
      ...
      const leaf=new THREE.Mesh(new THREE.ConeGeometry(.15*scale,.34*scale,6),mat(0x3aa64a,.68,.03)); leaf.position.set(0,1.44*scale,.14*scale); leaf.rotation.z=.08; g.add(leaf);
    ...
    case 'frostowl': {
      ...
      const beak=new THREE.Mesh(new THREE.ConeGeometry(.09*scale,.22*scale,5),mat(0xf6ad31,.7,.05)); beak.position.set(0,.98*scale,-.56*scale); beak.rotation.x=-Math.PI/2; g.add(beak);
    ...
    case 'galebird': {
      ...
      const beak2=new THREE.Mesh(new THREE.ConeGeometry(.08*scale,.2*scale,5),mat(0xf6ad31,.7,.05)); beak2.position.set(0,.99*scale,-.55*scale); beak2.rotation.x=-Math.PI/2; g.add(beak2);
      break;
    }
    case 'mindcoon': {
...
function monsterMesh(sp,owned=false,inst=null,eliteOverride=false,boss=false){
  ...
  const ring=new THREE.Mesh(new THREE.TorusGeometry(.58*ringScale,.045,8,28),new THREE.MeshBasicMaterial({color:ringColor}));
  ring.rotation.x=Math.PI/2; ring.position.y=.06; g.add(ring);
  if(eliteOverride||sp.elite||boss){
    const crest=new THREE.Mesh(new THREE.OctahedronGeometry((boss?.18:.13)*ringScale),mat(boss?0xffd166:0xfde047,.4,.15));
    crest.position.set(0,1.55*ringScale,0); crest.castShadow=true; g.add(crest);
  }
  return g;
}
function addBackpack(group,color=0x7c3aed){ const pack=new THREE.Mesh(new THREE.BoxGeometry(.24,.3,.14),mat(color,.74,.04)); pack.position.set(0,1.02,.23); pack.castShadow=true; group.add(pack); }
function addSatchel(group,color=0xb45309){ const satchel=new THREE.Mesh(new THREE.BoxGeometry(.18,.16,.08),mat(color,.74,.04)); satchel.position.set(-.26,.84,.18); satchel.rotation.z=.22; group.add(satchel); }
function addBoot(group,color,{x=0,y=-.04,z=0,w=.12,h=.06,d=.18}={}){ const boot=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),mat(color,.8,.02)); boot.position.set(x,y,z); group.add(boot); }
function buildHumanoid({shirt=0x20324a,pants=0x0f172a,skin=0xffc4a3,hair=0xf97316,hat=null,apron=null,bag=0x7c3aed,ballHand=true,staff=false}={}){
  const g=new THREE.Group();
  const torso=new THREE.Mesh(new THREE.CapsuleGeometry(.25,.58,6,12),mat(shirt,.72,.06)); torso.position.y=1.0; torso.castShadow=true; g.add(torso);
  const chest=new THREE.Mesh(new THREE.BoxGeometry(.36,.36,.24),mat(shirt,.72,.06)); chest.position.set(0,1.08,.02); chest.castShadow=true; g.add(chest);
  const belt=new THREE.Mesh(new THREE.BoxGeometry(.37,.05,.25),mat(0x111827,.82,.02)); belt.position.set(0,.76,.04); g.add(belt);
  const hips=new THREE.Mesh(new THREE.BoxGeometry(.38,.24,.25),mat(pants,.8,.04)); hips.position.y=.64; hips.castShadow=true; g.add(hips);
  const neck=new THREE.Mesh(new THREE.CylinderGeometry(.06,.07,.09,8),mat(skin,.72,.02)); neck.position.y=1.33; g.add(neck);
  const head=new THREE.Mesh(new THREE.SphereGeometry(.22,16,12),mat(skin,.72,.02)); head.position.y=1.56; head.castShadow=true; g.add(head);
  ...
  if(hat!==null){ const hatTop=new THREE.Mesh(new THREE.CylinderGeometry(.24,.28,.14,14),mat(hat,.62,.08)); hatTop.position.y=1.82; g.add(hatTop); const brim=new THREE.Mesh(new THREE.CylinderGeometry(.33,.35,.03,16),mat(hat,.62,.08)); brim.position.y=1.75; g.add(brim); }
  else { const hairCap=new THREE.Mesh(new THREE.SphereGeometry(.23,14,10,0,Math.PI*2,0,Math.PI*.6),mat(hair,.74,.02)); hairCap.position.set(0,1.68,-.01); hairCap.castShadow=true; g.add(hairCap); const bang=new THREE.Mesh(new THREE.BoxGeometry(.16,.08,.06),mat(hair,.74,.02)); bang.position.set(0,1.63,-.19); bang.rotation.x=.22; g.add(bang);} 
  if(apron!==null){ const ap=new THREE.Mesh(new THREE.BoxGeometry(.34,.44,.04),mat(apron,.78,.02)); ap.position.set(0,.95,-.14); g.add(ap); }
  ...
  const legL=new THREE.Mesh(new THREE.CylinderGeometry(.08,.09,.6,8),mat(pants,.78,.04)); legL.position.set(-.1,.26,.02); legL.castShadow=true; g.add(legL); addBoot(g,0x111827,{x:-.1,y:-.05,z:.05});
  const legR=new THREE.Mesh(new THREE.CylinderGeometry(.08,.09,.6,8),mat(pants,.78,.04)); legR.position.set(.1,.26,.02); legR.castShadow=true; g.add(legR); addBoot(g,0x111827,{x:.1,y:-.05,z:.05});
  addBackpack(g,bag); addSatchel(g,0x8b5e34);
  let staffRig=null;
  if(staff){ const rod=new THREE.Mesh(new THREE.CylinderGeometry(.028,.03,.72,8),mat(0x475569,.85,.02)); rod.position.set(-.48,.8,.02); rod.rotation.z=.32; rod.castShadow=true; g.add(rod); const orbTop=orb(0xf59e0b,.055,8); orbTop.position.set(-.59,1.1,.03); g.add(orbTop); staffRig={rod,orbTop}; }
...
// ---------- Player / NPC ----------
const player=buildPlayerCharacter(); scene.add(player); player.position.set(0,0,5);
const playerData={hp:100,maxHp:100,speed:5.7,invuln:0};

const npc=buildKeeperCharacter(); npc.position.set(4,0,3); scene.add(npc);
const effects=[];
...
function animateHumanoid(model,dt,moving=false){
  ...
  if(rig.staffRig){ rig.staffRig.rod.rotation.z=.32+Math.sin(rig.phase*.7)*.02; rig.staffRig.orbTop.material.emissive=new THREE.Color(0xf59e0b); rig.staffRig.orbTop.material.emissiveIntensity=.22+.08*Math.abs(Math.sin(rig.phase)); }
}
...
function spawnBurst(pos,color=0x8b5cf6,{count=8,life=.4,size=.06,gravity=0}={}){
  for(let i=0;i<count;i++){ const m=orb(color,size,8); m.material.transparent=true; m.material.opacity=.9; m.position.copy(pos); scene.add(m); const a=(Math.PI*2*i)/count + Math.random()*.2; const speed=.8+Math.random()*1.6; effects.push({mesh:m,vel:new THREE.Vector3(Math.cos(a)*speed,.4+Math.random()*1.2,Math.sin(a)*speed),life,maxLife:life,kind:'spark',gravity}); }
}
function spawnRingPulse(pos,color=0x60a5fa,{scale=.6,life=.35,y=.08}={}){ const mesh=new THREE.Mesh(new THREE.TorusGeometry(scale,.03,8,28),new THREE.MeshBasicMaterial({color,transparent:true,opacity:.9})); mesh.rotation.x=Math.PI/2; mesh.position.copy(pos); mesh.position.y+=y; scene.add(mesh); effects.push({mesh,life,maxLife:life,kind:'ring'}); }
function updateEffects(dt){ for(let i=effects.length-1;i>=0;i--){ const e=effects[i]; e.life-=dt; const t=Math.max(0,e.life/e.maxLife); if(e.kind==='spark'){ e.vel.y-=(e.gravity||0)*dt; e.mesh.position.addScaledVector(e.vel,dt); e.mesh.scale.setScalar(.5+t); } else if(e.kind==='ring'){ e.mesh.scale.multiplyScalar(1+dt*2.8); } e.mesh.material.opacity=Math.max(0,t*.9); if(e.life<=0){ scene.remove(e.mesh); effects.splice(i,1);} } }
...
function fxGeom(shape='orb',size=0.06){
  switch(shape){
    case 'flame': return new THREE.ConeGeometry(size*0.7,size*1.8,6);
    case 'drop': return new THREE.SphereGeometry(size,10,8);
    case 'leaf': return new THREE.OctahedronGeometry(size*0.9,0);
    case 'crystal': return new THREE.OctahedronGeometry(size,0);
    case 'impact': return new THREE.BoxGeometry(size*1.5,size*0.45,size*1.2);
    case 'bubble': return new THREE.SphereGeometry(size*0.85,10,8);
    case 'dust': return new THREE.BoxGeometry(size*1.2,size*0.55,size*1.2);
    case 'feather': return new THREE.ConeGeometry(size*0.55,size*1.7,4);
    case 'halo': return new THREE.TorusGeometry(size*0.82,size*0.18,6,12);
    case 'spore': return new THREE.SphereGeometry(size*0.72,8,6);
    case 'shard': return new THREE.TetrahedronGeometry(size*0.95,0);
    case 'mist': return new THREE.SphereGeometry(size*0.95,8,6);
    case 'arc': return new THREE.ConeGeometry(size*0.65,size*1.9,5);
    case 'smoke': return new THREE.SphereGeometry(size,8,6);
    case 'metal': return new THREE.BoxGeometry(size,size*0.52,size*1.45);
    case 'star': return new THREE.OctahedronGeometry(size*0.9,0);
    case 'spark': return new THREE.BoxGeometry(size*1.5,size*0.35,size*1.5);
    default: return new THREE.SphereGeometry(size,10,8);
  }
}
function spawnElementalFX(type,pos,mode='impact',power=1){
  const cfg=typeFx(type),base=pos.clone();
...
function setActionStyle(btn,type,label,sub){
  ...
  btn.style.setProperty('--action-main','#'+cfg.core.toString(16).padStart(6,'0'));
  btn.style.setProperty('--action-accent','#'+cfg.accent.toString(16).padStart(6,'0'));
...
function spawnDamageNumber(amount,pos,{type='Normal',eff=1,healing=false,label=''}={}){
  ...
  layer.appendChild(d);
  floatingTexts.push({el:d,pos:pos.clone(),life:0.9,maxLife:0.9,rise:0,drift:(Math.random()-.5)*24});
}
function updateFloatingTexts(dt){
  for(let i=floatingTexts.length-1;i>=0;i--){
    const f=floatingTexts[i]; f.life-=dt; f.rise+=dt*1.1;
    const p=f.pos.clone().add(new THREE.Vector3(0,f.rise,0));
...
function spawnGroundDecal(type,pos,{radius=1.1,duration=1.25,intensity=1}={}){
  ...
  const disc=new THREE.Mesh(new THREE.CircleGeometry(radius,36),new THREE.MeshBasicMaterial({color:cfg.core,transparent:true,opacity:0.13*intensity,side:THREE.DoubleSide,depthWrite:false,blending:THREE.AdditiveBlending}));
  disc.rotation.x=-Math.PI/2; disc.position.y=.032; group.add(disc);
  const ring=new THREE.Mesh(new THREE.RingGeometry(radius*.64,radius*.78,40),new THREE.MeshBasicMaterial({color:cfg.accent,transparent:true,opacity:0.42*intensity,side:THREE.DoubleSide,depthWrite:false,blending:THREE.AdditiveBlending}));
  ring.rotation.x=-Math.PI/2; ring.position.y=.038; group.add(ring);
  const inner=new THREE.Mesh(new THREE.RingGeometry(radius*.18,radius*.24,28),new THREE.MeshBasicMaterial({color:cfg.core,transparent:true,opacity:0.48*intensity,side:THREE.DoubleSide,depthWrite:false,blending:THREE.AdditiveBlending}));
...
function updateGroundDecals(dt){
  for(let i=groundDecals.length-1;i>=0;i--){
    ...
    if(d.life<=0){scene.remove(d.group);groundDecals.splice(i,1);}
  }
}


// ---------- State / save ----------
const saveKey='monster-life-rpg-proto-v6';
const oldV5Key='monster-life-rpg-proto-v5';
const oldV4Key='monster-capture-summon-proto-v4';
const state={collection:[],party:[null,null,null],storage:[],ranchActive:[],selectedSlot:0,exp:0,lifeLastAt:Date.now(),inventory:{captureBalls:12,protein:6,healthy:6,favorite:6},eggs:[],breeding:{parentA:null,parentB:null},evolutionCandidate:null,currentZone:'hub',saveVersion:6};
...
let nextId=1;const wilds=[],projectiles=[];let activeSummon=null;let pendingSummon=null;let summonCooldownUntil=0;
...
function createWild(sp,x,z,level=1,opts={}){
  ...
  const marker=new THREE.Mesh(new THREE.OctahedronGeometry(boss?.22:.16),new THREE.MeshStandardMaterial({color:markerColor,emissive:markerColor,emissiveIntensity:.65,roughness:.35}));
...
function respawnWild(w,delay=6000){const zone=w.zone,id=w.speciesId,level=w.level,opts={boss:w.boss,elite:w.elite};setTimeout(()=>{if(state.currentZone!==zone)return;const cfg=ZONES[zone];if(!cfg)return;const p={x:w.home.x,z:w.home.z};createWild(spById[id],p.x,p.z,level,opts);},delay);}
function clearProjectiles(){ while(projectiles.length){ const p=projectiles.pop(); scene.remove(p.mesh); } pendingSummon=null; }
...
function clearHubCompanion(){
  if(hubCompanion) scene.remove(hubCompanion.mesh);
  hubCompanion=null;
...
const joyEl=el('joystick'); if(!joyEl) throw new Error('V7.0.5 boot: #joystick not found');
const stick=el('stick'); if(!stick) throw new Error('V7.0.5 boot: #stick not found');
...
function updateWorldLabels(){
  for(const w of wilds){
    ...
    const d=distXZ(player.position,w.mesh.position),screen=worldToScreen(w.mesh.position.clone().add(new THREE.Vector3(0,w.boss?2.55:2.15,0)));
...
function defeatWild(w){if(w.dead)return;w.dead=true;scene.remove(w.mesh);removeWildLabel(w);state.exp+=12*w.level;const monGain=activeSummon?8*w.level:0,ups=activeSummon?grantMonsterExp(activeSummon.inst,monGain):0;const tag=w.boss?'BOSS ':w.elite?'ELITE ':'';msg(`${tag}${wildDisplayName(w)} ถูกปราบ +${12*w.level} EXP${activeSummon?` • ${displayName(activeSummon.inst)} +${monGain} EXP${ups?` • Level Up +${ups}`:''}`:''}`);renderAll();saveGame(false);respawnWild(w,w.boss?BALANCE.bossRespawnMs:BALANCE.wildRespawnMs);}
function monsterDamage(attackerInst,move,defender,atkBuff=1){const sp=spById[attackerInst.speciesId],atk=(attackerInst.atk||sp.base.atk)*atkBuff,def=defender.def||10,stab=monsterTypes(attackerInst).includes(move.type)?1.5:1,defTypes=defender?.instanceId?monsterTypes(defender):wildTypes(defender),eff=typeEffectiveness(move.type,defTypes),base=(move.power+atk*.75)*(1+(attackerInst.level-1)*.025),mitigation=100/(100+def*3.2),traitBonus=attackerInst.genes?.trait==='Fierce'?1.08:1;return{damage:Math.max(eff===0?0:1,Math.round(base*mitigation*stab*eff*traitBonus)),stab,eff};}
function wildDamage(w,inst){const sp=spById[w.speciesId],move={type:sp.types[0],power:w.boss?24:18},def=inst.def||10,stab=1.5,eff=typeEffectiveness(move.type,monsterTypes(inst)),base=(move.power+w.atk*.6)*(100/(100+def*3));return{damage:Math.max(eff===0?0:1,Math.round(base*stab*eff)),eff};}
function throwProjectile(type,targetPos,onHit){const color=type==='capture'?0x3b82f6:0x8b5cf6,mesh=new THREE.Mesh(new THREE.SphereGeometry(.16,12,10),new THREE.MeshStandardMaterial({color,emissive:color,emissiveIntensity:.45,transparent:true,opacity:.96}));mesh.position.copy(player.position).add(new THREE.Vector3(0,1.15,0));mesh.castShadow=true;scene.add(mesh);spawnBurst(mesh.position.clone(),color,{count:5,life:.18,size:.04});projectiles.push({mesh,type,color,start:mesh.position.clone(),end:targetPos.clone(),t:0,duration:.55,onHit,lastTrail:0});}
function captureChance(w){if(w.capturePolicy==='disabled')return 0;const sp=spById[w.speciesId],hpFactor=.42+(1-w.hp/w.maxHp)*1.18,policyMod=w.capturePolicy==='elite'?BALANCE.eliteCaptureModifier:1;return Math.min(.94,sp.capture*hpFactor*policyMod);}
...
function beginCaptureAim(){if(!capturePrerequisite())return false;captureAimActive=true;captureAimLine.visible=true;el('captureBtn').classList.add('aiming');msg('Capture Aim • ลากด้านขวาหมุนกล้อง แล้วปล่อยปุ่มเพื่อขว้าง');return true;}
function cancelCaptureAim(){captureAimActive=false;captureAimLine.visible=false;el('captureBtn').classList.remove('aiming');}
function updateCaptureAimVisual(){if(!captureAimActive)return;const t=aimedWild(BALANCE.captureRange,BALANCE.captureAimRadius),start=player.position.clone().add(new THREE.Vector3(0,1.15,0)),end=t?t.mesh.position.clone().add(new THREE.Vector3(0,.65,0)):player.position.clone().add(forward().multiplyScalar(8)).add(new THREE.Vector3(0,.15,0)),pts=[];for(let i=0;i<=18;i++){const u=i/18,p=start.clone().lerp(end,u);p.y+=Math.sin(u*Math.PI)*2.2;pts.push(p);}captureAimGeom.setFromPoints(pts);}
function executeCaptureThrow(){if(!captureAimActive)return;captureAimActive=false;captureAimLine.visible=false;el('captureBtn').classList.remove('aiming');if(!capturePrerequisite())return;setHumanoidAction(player,'throw',.34);state.inventory.captureBalls--;const t=aimedWild(BALANCE.captureRange,BALANCE.captureAimRadius),end=t?t.mesh.position.clone().add(new THREE.Vector3(0,.65,0)):player.position.clone().add(forward().multiplyScalar(8)).add(new THREE.Vector3(0,.15,0));throwProjectile('capture',end,()=>{if(!t||t.dead){msg('ปาพลาด/ลูกตกพื้น • เสีย Capture Ball 1 ลูก');renderHUD();saveGame(false);return;}resolveCapture(t);});if(t){msg(`ปา Capture Ball → ${t.boss?'BOSS ':t.elite?'ELITE ':''}${wildDisplayName(t)}`);}else msg('ปา Capture Ball ตามจุดเล็ง…');renderHUD();saveGame(false);}
function captureThrow(){if(beginCaptureAim())executeCaptureThrow();}
function resolveCapture(w){
  if(w.dead)return;const sp=spById[w.speciesId],name=wildDisplayName(w);if(w.capturePolicy==='disabled'){msg(`Boss ${name} จับไม่ได้ในเวอร์ชันนี้ • บอลถูกใช้ไปแล้ว`);return;}const chance=captureChance(w);
  if(Math.random()<chance){spawnBurst(w.mesh.position.clone().add(new THREE.Vector3(0,.7,0)),0x22c55e,{count:14,life:.42,size:.06});spawnRingPulse(w.mesh.position.clone(),0x22c55e,{scale:.75,life:.3});spawnGroundDecal(wildTypes(w)[0],w.mesh.position.clone(),{radius:1.1,duration:.8,intensity:.75});triggerCameraShake(.08,.12);const inst=makeInstance(sp,w.level,{origin:'captured',genes:w.genes,gender:w.gender,bond:24,evolutionPath:w.evolutionPath,secondaryType:wildPath(w)?.secondaryType??sp.types[1]??null});state.collection.push(inst);const empty=state.party.findIndex(x=>x===null);if(empty>=0)state.party[empty]=inst.instanceId;else state.storage.push(inst.instanceId);w.dead=true;scene.remove(w.mesh);removeWildLabel(w);state.exp+=5;msg(`จับ ${name} สำเร็จ! ${empty>=0?'เข้า Party ช่อง '+(empty+1):'ส่งเข้า Storage'}${w.elite?' • ELITE':''}`);renderAll();saveGame(false);respawnWild(w,8000);}else{msg(`จับ ${name} ไม่สำเร็จ (${Math.round(chance*100)}%) • บอลถูกใช้ไปแล้ว`);w.engaged=true;w.state='chase';}
}
function summonThrow(){const inst=selectedInstance();if(Date.now()<summonCooldownUntil){msg(`Switch cooldown ${(summonCooldownUntil-Date.now())/1000|0}s`);return;}if(state.currentZone==='hub'){msg('ใน Ranch จะแสดงคู่หูอัตโนมัติ • ออกไป Wild Zone ก่อนแล้วค่อยปาเรียก');return;}if(!inst){msg('Party ช่องนี้ว่าง');return;}if(activeSummon||pendingSummon){msg('ลงสนามได้ครั้งละ 1 ตัว • Recall ตัวเดิมก่อน');return;}if(inst.hp<=0||inst.fainted){msg(`${displayName(inst)} Fainted • Heal ฟรีที่ Ranch/NPC ก่อน`);return;}const end=player.position.clone().add(forward().multiplyScalar(4));end.y=.12;setHumanoidAction(player,'throw',.34);pendingSummon={instanceId:inst.instanceId};clearHubCompanion();throwProjectile('summon',end,()=>{if(!pendingSummon||pendingSummon.instanceId!==inst.instanceId)return;pendingSummon=null;spawnOwned(inst,end);});msg(`ปาเรียก ${displayName(inst)}`);}
...
function recall(show=true,setCooldown=true){
  ...
  setHumanoidAction(player,'recall',.28);
  ...
  scene.remove(activeSummon.mesh);
  activeSummon=null;
  removeSceneRole('activeSummon');
  syncHubCompanion();
...
function faintActive(){if(!activeSummon){removeSceneRole('activeSummon');return;}const inst=activeSummon.inst;inst.hp=0;inst.fainted=true;const name=displayName(inst);spawnBurst(activeSummon.mesh.position.clone().add(new THREE.Vector3(0,.6,0)),0xef4444,{count:12,life:.32,size:.06,gravity:1.2});scene.remove(activeSummon.mesh);activeSummon=null;pendingSummon=null;removeSceneRole('activeSummon');syncHubCompanion();summonCooldownUntil=Date.now()+800;msg(`${name} Fainted • Auto Recall • ต้อง Heal ที่ Ranch/NPC หรือ Item`);renderParty();renderSkillButtons();renderHUD();saveGame(false);}
...
function updateWild(w,dt){
  if(w.dead)return;w.attackCd=Math.max(0,w.attackCd-dt);w.wanderT-=dt;let target=activeSummon?activeSummon.mesh:player,dd=distXZ(w.mesh.position,target.position);if(dd>14&&!w.engaged){if(w.wanderT<=0){w.state='wander';w.wanderT=1.6+Math.random()*2.2;w.dir=new THREE.Vector3((Math.random()-.5)*2,0,(Math.random()-.5)*2).normalize();}let moving=false;if(w.state==='wander'){const next=w.mesh.position.clone().add(w.dir.clone().multiplyScalar(dt*.9));if(distXZ(next,w.home)>2.2)w.dir.multiplyScalar(-1);w.mesh.position.addScaledVector(w.dir,dt*.9);w.mesh.rotation.y=monsterLookYaw(w.dir,w.mesh);moving=true;}animateEntity(w.mesh,dt,moving,w.boss?1.2:1);animateMonster(w.mesh,dt,moving);return;}
  w.engaged=true;let moving=false;if(target){const d=distXZ(w.mesh.position,target.position);
    if(d>1.25){moving=true;const dir=target.position.clone().sub(w.mesh.position);dir.y=0;dir.normalize();w.mesh.position.addScaledVector(dir,(w.spd*.16+1.2)*dt);w.mesh.rotation.y=monsterLookYaw(dir,w.mesh);}else if(w.attackCd<=0){w.attackCd=w.boss?.85:1.2;triggerMonsterAction(w.mesh,'attack',0.22);spawnElementalFX(wildTypes(w)[0],target.position.clone().add(new THREE.Vector3(0,.55,0)),'impact',0.65);if(activeSummon){const res=wildDamage(w,activeSummon.inst),reduction=activeSummon.shieldTimer>0?activeSummon.shieldReduction:0,dmg=Math.round(res.damage*(1-reduction));activeSummon.inst.hp-=dmg;spawnDamageNumber(dmg,activeSummon.mesh.position.clone().add(new THREE.Vector3(0,1.25,0)),{type:wildTypes(w)[0],eff:res.eff});triggerCameraShake(w.boss?.15:.09,w.boss?.2:.14);if(activeSummon.inst.hp<=0)faintActive();renderParty();}else if(playerData.invuln<=0){const playerDmg=Math.round(w.atk*(w.boss?.7:.48));playerData.hp-=playerDmg;playerData.invuln=.5;spawnDamageNumber(playerDmg,player.position.clone().add(new THREE.Vector3(0,1.45,0)),{type:wildTypes(w)[0]});triggerCameraShake(w.boss?.17:.1,w.boss?.22:.15);setHumanoidAction(player,'hurt',.24);spawnBurst(player.position.clone().add(new THREE.Vector3(0,1,0)),0xef4444,{count:8,life:.22,size:.04,gravity:1});if(playerData.hp<=0){playerData.hp=playerData.maxHp;msg('ผู้เล่นหมด HP • กลับ Ranch Hub');switchZone('hub',true);}renderHUD();}}}
  animateEntity(w.mesh,dt,moving,w.boss?1.2:1); animateMonster(w.mesh,dt,moving);
}
function updateProjectiles(dt){for(let i=projectiles.length-1;i>=0;i--){const p=projectiles[i];p.t+=dt/p.duration;const t=Math.min(1,p.t);p.mesh.position.lerpVectors(p.start,p.end,t);p.mesh.position.y+=Math.sin(t*Math.PI)*2.2;if(t-p.lastTrail>.08){p.lastTrail=t;if(p.type==='summon'&&pendingSummon){const inst=getInst(pendingSummon.instanceId);if(inst)spawnElementalFX(monsterTypes(inst)[0],p.mesh.position.clone(),'trail',0.55);}else{spawnBurst(p.mesh.position.clone(),p.color,{count:3,life:.12,size:.03});}}if(t>=1){spawnBurst(p.end.clone(),p.color,{count:6,life:.16,size:.04});scene.remove(p.mesh);projectiles.splice(i,1);p.onHit?.();}}}

// ---------- V7.0.5 restored Ranch / life / training core ----------
...
function syncRanchVisuals(){
  for(const [id,obj] of ranchVisuals){scene.remove(obj.mesh);ranchVisuals.delete(id);}
  removeSceneRole('ranchVisual');
...
function makeChild(a,b,holder){const sp=resolveBreedingSpecies(a,b,holder),other=holder===a?b:a,otherTypes=monsterTypes(other);let secondary=sp.types[1]||null;if(!secondary&&Math.random()<.35){const candidate=otherTypes.find(t=>t!==sp.types[0]&&sp.allowedSecondary.includes(t));if(candidate)secondary=candidate;}const genes={hp:chooseInheritedPotential(a,b,'hp'),atk:chooseInheritedPotential(a,b,'atk'),def:chooseInheritedPotential(a,b,'def'),spd:chooseInheritedPotential(a,b,'spd'),trait:Math.random()<.5?a.genes.trait:b.genes.trait,skillGene:Math.random()<.5?a.genes.skillGene:b.genes.skillGene,typeAffinity:Math.random()<.5?a.genes.typeAffinity:b.genes.typeAffinity};const mother=a.gender==='Female'?a:(b.gender==='Female'?b:holder),father=mother===a?b:a;const child=makeInstance(sp,1,{origin:'bred',genes,bond:12,hunger:90,energy:90,mood:88,fitness:42,generation:Math.max(a.generation||1,b.generation||1)+1,parentAId:a.instanceId,parentBId:b.instanceId,motherId:mother?.instanceId||null,fatherId:father?.instanceId||null,secondaryType:secondary});child.lifeStage='Baby';return child;}
function createEgg(){const a=getInst(state.breeding.parentA),b=getInst(state.breeding.parentB),compat=breedingCompatibility(a,b);if(!compat.ok){msg(compat.text);renderBreeding();return;}const holder=eggHolder(a,b);if(!holder){msg('ไม่พบผู้ถือไข่ที่เข้ากันได้');return;}a.energy=clamp(a.energy-15);b.energy=clamp(b.energy-15);const hatchMs=30000+Math.floor(Math.random()*30001),egg={eggId:'e'+Date.now()+'-'+Math.floor(Math.random()*9999),parentAId:a.instanceId,parentBId:b.instanceId,eggHolderId:holder.instanceId,createdAt:Date.now(),readyAt:Date.now()+hatchMs};state.eggs.push(egg);msg(`สร้างไข่สำเร็จ • ผู้ถือไข่ ${displayName(holder)} • ฟัก ${Math.round(hatchMs/1000)} วินาที`);renderManager();saveGame(false);}
function hatchEgg(eggId){const egg=state.eggs.find(e=>e.eggId===eggId);if(!egg)return;if(Date.now()<egg.readyAt){msg('ไข่ยังไม่พร้อมฟัก');return;}const a=getInst(egg.parentAId),b=getInst(egg.parentBId),holder=getInst(egg.eggHolderId);if(!a||!b||!holder){msg('ข้อมูลพ่อแม่/ผู้ถือไข่ไม่ครบ');return;}const child=makeChild(a,b,holder);state.collection.push(child);state.storage.push(child.instanceId);state.eggs=state.eggs.filter(e=>e.eggId!==eggId);msg(`ฟักไข่! ได้ ${displayName(child)} Gen ${child.generation} • Species ตามผู้ถือไข่ ${displayName(holder)}`);renderAll();renderManager();saveGame(false);}

// ---------- Evolution ----------
function evoRequirementStatus(inst,path){const req=path.requires||{},items=[];let ok=true;if(req.level){const x=inst.level>=req.level;ok&&=x;items.push(`${x?'✓':'✗'} Lv.${req.level}`);}if(req.bond){const x=inst.bond>=req.bond;ok&&=x;items.push(`${x?'✓':'✗'} Bond ≥ ${req.bond}`);}if(req.trainingFocus){const x=inst.trainingFocus===req.trainingFocus;ok&&=x;items.push(`${x?'✓':'✗'} Training ${TRAIN_FOCUS[req.trainingFocus]}`);}if(req.environment){const x=state.currentZone===req.environment;ok&&=x;items.push(`${x?'✓':'✗'} Environment ${ZONES[req.environment]?.label||req.environment}`);}return{ok,text:items.join(' • ')||'ไม่มีเงื่อนไข'};}
function showEvolution(id){state.evolutionCandidate=id;renderEvolution();}
function evolveMonster(id,pathId){const inst=getInst(id),sp=spById[inst?.speciesId],path=sp?.evolutionPaths?.find(p=>p.id===pathId);if(!inst||!path||inst.evolutionPath)return;const st=evoRequirementStatus(inst,path);if(!st.ok){msg('ยังไม่ผ่านเงื่อนไข Evolution');return;}if(!confirm(`Evolution ย้อนกลับไม่ได้\n${displayName(inst)} → ${path.name}\nยืนยันหรือไม่?`))return;inst.evolutionPath=path.id;if(path.secondaryType&&sp.allowedSecondary.includes(path.secondaryType))inst.secondaryType=path.secondaryType;refreshStats(inst,true);msg(`${sp.name} Evolution → ${path.name} สำเร็จ!`);syncRanchVisuals();renderAll();renderManager();saveGame(false);}
function renderEvolution(){const box=el('evolutionPreview'),inst=getInst(state.evolutionCandidate);if(!inst){box.innerHTML='เลือก “ดู Evolution” จากการ์ดมอนเพื่อดูเส้นทาง';return;}const sp=spById[inst.speciesId];if(inst.evolutionPath){const p=getEvolutionPath(inst);box.innerHTML=`<div class="evo-card"><div class="evo-title"><b>${displayName(inst)}</b><span>Evolution Locked</span></div><div class="evo-details">วิวัฒนาการแล้วและย้อนกลับไม่ได้ • Type ${monsterTypes(inst).map(t=>TYPE_TH[t]).join('/')} • HP ${inst.maxHp} ATK ${inst.atk} DEF ${inst.def} SPD ${inst.spd}</div></div>`;return;}if(!sp.evolutionPaths?.length){box.innerHTML=`<div class="evo-card">${sp.name} ยังไม่มี Evolution Path ใน Vertical Slice นี้</div>`;return;}box.innerHTML='';for(const p of sp.evolutionPaths){const st=evoRequirementStatus(inst,p),types=[sp.types[0],p.secondaryType??inst.secondaryType??sp.types[1]].filter(Boolean),mods=p.statMods||{},skills=getMonsterSkills(inst).map(s=>s.name).join(', '),d=document.createElement('div');d.className='evo-card';d.innerHTML=`<div class="evo-title"><b>${sp.name} → ${p.name}</b><span>${st.ok?'พร้อม':'ยังไม่พร้อม'}</span></div><div class="evo-details">รูปร่าง: ${p.name} (Form/สี/ขนาดใหม่)<br>Type: ${types.map(t=>TYPE_TH[t]).join(' / ')} — Primary ${TYPE_TH[sp.types[0]]} ล็อกตาม Species<br>Stats: HP ×${mods.hp||1} • ATK ×${mods.atk||1} • DEF ×${mods.def||1} • SPD ×${mods.spd||1}<br>Skills หลัง Evolution: ${skills}<br>เงื่อนไข: ${st.text}</div><button ${st.ok?'':'disabled'}>${st.ok?'ยืนยัน Evolution':'เงื่อนไขยังไม่ครบ'}</button>`;d.querySelector('button').onclick=()=>evolveMonster(inst.instanceId,p.id);box.appendChild(d);}}
...
function openManager(){if(!isNearNpc()){msg('กลับ Ranch Hub และเข้าใกล้ NPC ก่อน');return;}if(ensureCaptureBallSafety())msg('Keeper Starter Kit • Capture Ball +5');applyLifeSimulation(Date.now(),true);el('monsterManager').classList.remove('hidden');setManagerTab(currentManagerTab||'collection');renderManager();}
...
function renderBreeding(){
  const ids=breedingAdultIds();if(state.breeding.parentA&&!ids.includes(state.breeding.parentA))state.breeding.parentA=null;if(state.breeding.parentB&&!ids.includes(state.breeding.parentB))state.breeding.parentB=null;
  ...
  el('parentABtn').innerHTML=parentButtonHTML(a);el('parentBBtn').innerHTML=parentButtonHTML(b);c.textContent=compat.text;c.className='compatibility '+(a&&b?(compat.ok?'ok':'bad'):'');el('breedBtn').disabled=!compat.ok;
  if(a&&b){const holder=eggHolder(a,b);el('inheritPreview').innerHTML=`ผู้ถือไข่: <b>${holder?displayName(holder):'ไม่พบ'}</b> → Species ตามผู้ถือไข่ • Gene จากทั้งสองตัว • Trait <b>${a.genes.trait}</b> / <b>${b.genes.trait}</b>`;}else el('inheritPreview').textContent='เลือกพ่อแม่เพื่อดูแนวโน้ม Gene ที่ส่งต่อ';
  const list=el('eggList');list.innerHTML='';if(!state.eggs.length)list.innerHTML='<div class="manager-empty">ยังไม่มีไข่ใน Incubator</div>';for(const egg of state.eggs){const pa=getInst(egg.parentAId),pb=getInst(egg.parentBId),holder=getInst(egg.eggHolderId),remain=Math.max(0,Math.ceil((egg.readyAt-Date.now())/1000)),d=document.createElement('div');d.className='egg-card';d.innerHTML=`<div class="egg-top"><span class="egg-icon">🥚</span><b>${remain>0?remain+'s':'พร้อมฟัก!'}</b></div><div class="egg-meta">${pa?displayName(pa):'?'} × ${pb?displayName(pb):'?'}<br>ผู้ถือไข่: ${holder?displayName(holder):'?'}</div><button ${remain>0?'disabled':''}>${remain>0?'กำลังฟัก':'ฟักไข่'}</button>`;d.querySelector('button').onclick=()=>hatchEgg(egg.eggId);list.appendChild(d);}
}
function renderManager(){applyLifeSimulation(Date.now());const partyBox=el('managerParty'),storageBox=el('managerStorage');partyBox.innerHTML='';storageBox.innerHTML='';const partyIds=state.party.filter(Boolean);el('partyCountLabel').textContent=`${partyIds.length}/3`;el('storageCountLabel').textContent=`${state.storage.length}`;el('ranchActiveCountLabel').textContent=`${state.ranchActive.length}/${RANCH_ACTIVE_MAX}`;if(!partyIds.length)partyBox.innerHTML='<div class="manager-empty">Party ว่าง</div>';partyIds.forEach(id=>{const i=getInst(id);if(i)partyBox.appendChild(monsterCard(i,'party'));});if(!state.storage.length)storageBox.innerHTML='<div class="manager-empty">Storage ว่าง</div>';state.storage.forEach(id=>{const i=getInst(id);if(i)storageBox.appendChild(monsterCard(i,'storage'));});el('foodProtein').textContent=state.inventory.protein||0;el('foodHealthy').textContent=state.inventory.healthy||0;el('foodFavorite').textContent=state.inventory.favorite||0;el('managerBallCount').textContent=state.inventory.captureBalls||0;renderEvolution();renderBreeding();}

// ---------- HUD / rendering ----------
function renderHUD(){el('playerHp').textContent=`${fmt(playerData.hp)}/${playerData.maxHp}`;el('collectionCount').textContent=state.collection.length;const balls=state.inventory.captureBalls||0;el('captureBallCount').textContent=balls;document.querySelector('.ball-pill')?.classList.toggle('warning',balls<=2);el('playerExp').textContent=Math.floor(state.exp);el('ranchCount').textContent=`${state.ranchActive.length}/${RANCH_ACTIVE_MAX}`;el('zoneLabel').textContent=ZONES[state.currentZone]?.label||state.currentZone;const wc=el('wildCount');if(wc)wc.textContent=state.currentZone==='hub'?'0':livingWilds().length;const a=el('activeMonsterStatus');if(a){if(activeSummon)a.textContent=displayName(activeSummon.inst);else if(pendingSummon)a.textContent='กำลังเรียก…';else if(hubCompanion)a.textContent=`${displayName(hubCompanion.inst)} • Ranch`;else a.textContent='ยังไม่เรียก';}}
function renderParty(){const p=el('party');p.innerHTML='';state.party.forEach((id,i)=>{const inst=getInst(id),d=document.createElement('button');d.className='party-slot compact'+(i===state.selectedSlot?' active':'')+(!inst?' empty':'');if(inst){const sp=spById[inst.speciesId],pct=hpPct(inst.hp/inst.maxHp)*100;d.title=`${displayName(inst)} • Lv.${inst.level} • Bond ${fmt(inst.bond)} • HP ${fmt(inst.hp)}/${inst.maxHp}`;d.innerHTML=`<span class="party-portrait" style="background:#${sp.color.toString(16).padStart(6,'0')}">${displayName(inst).slice(0,1)}</span><span class="party-mini"><b>${i+1}. ${displayName(inst)}</b><span class="mini-hp"><i style="width:${pct}%"></i></span><small class="${inst.fainted?'fainted':''}">${inst.fainted?'FAINTED':'HP '+fmt(inst.hp)+'/'+inst.maxHp}</small></span>`;}else d.innerHTML=`<span class="party-portrait empty-portrait">＋</span><span class="party-mini"><b>${i+1}. ว่าง</b><small>จับมอนเพิ่ม</small></span>`;d.onclick=()=>{state.selectedSlot=i;syncHubCompanion();renderParty();renderSkillButtons();renderHUD();};p.appendChild(d);});}
function renderSkillButtons(){const inst=activeSummon?.inst||selectedInstance(),skills=inst?getMonsterSkills(inst):[],activeType=inst?monsterTypes(inst)[0]:'Normal';[0,1,2].forEach(i=>{const b=el(`skill${i+1}Btn`),s=skills[i],cd=activeSummon?.skillCds?.[i]||0;b.disabled=!activeSummon||!s;b.classList.toggle('cooldown',cd>0);b.innerHTML=s?`<span>S${i+1}${cd>0?' '+cd.toFixed(1)+'s':''}</span><small>${s.name}</small>`:`<span>S${i+1}</span><small>สกิล ${i+1}</small>`;setActionStyle(b,s?.type||activeType,`S${i+1}`,s?.name||`สกิล ${i+1}`);});setActionStyle(el('captureBtn'),'Water','CAP','Capture');setActionStyle(el('summonBtn'),activeType,'SUM','Summon');setActionStyle(el('recallBtn'),'Psychic','REC','Recall');}
function updateTarget(){const t=activeSummon?.target&&!activeSummon.target.dead?activeSummon.target:(aimedWild(10,1.8)||nearestWild(10)),card=el('targetCard');if(!t){card.classList.add('hidden');return;}card.classList.remove('hidden');const sp=spById[t.speciesId],tag=t.boss?' ★ BOSS':t.elite?' ★ ELITE':'';el('targetName').textContent=sp.name+tag;el('targetLevel').textContent=`Lv.${t.level}`;el('targetHpText').textContent=`HP ${fmt(t.hp)}/${t.maxHp}`;el('targetHpBar').style.width=`${hpPct(t.hp/t.maxHp)*100}%`;el('targetTypes').innerHTML=sp.types.map(typeBadge).join('');const hint=el('typeHint');if(activeSummon){const move=getMonsterSkills(activeSummon.inst)[0],mult=typeEffectiveness(move.type,sp.types),[txt,cls]=effectLabel(mult);hint.textContent=`S1 ${TYPE_TH[move.type]}: ${txt}`;hint.className='tiny effect '+cls;}else if(t.boss){hint.textContent='BOSS • จับไม่ได้';hint.className='tiny effect weak';}else{hint.textContent=`${t.elite?'ELITE • ':''}จับ ~${Math.round(captureChance(t)*100)}%`;hint.className='tiny effect none';}}
function renderZoneUI(){document.querySelectorAll('[data-zone]').forEach(b=>b.classList.toggle('active',b.dataset.zone===state.currentZone));const hunt=el('huntBtn');if(hunt){if(state.currentZone==='hub'){hunt.textContent='ออกล่า → ทุ่ง • Wild 6';hunt.classList.remove('return');}else{hunt.textContent='← กลับ Ranch';hunt.classList.add('return');}}document.body.dataset.zone=state.currentZone;renderHUD();}
function renderAll(){renderHUD();renderParty();renderSkillButtons();updateTarget();renderZoneUI();}

// ---------- Save migration ----------
function migrateLoadedState(s){if(!s)return;state.collection=(s.collection||[]).map(ensureInstanceShape);state.party=(s.party||[null,null,null]).slice(0,3);while(state.party.length<3)state.party.push(null);state.storage=s.storage||[];state.ranchActive=(s.ranchActive||[]).filter(id=>state.storage.includes(id)).slice(0,RANCH_ACTIVE_MAX);state.selectedSlot=Math.min(2,s.selectedSlot||0);state.exp=s.exp||0;state.lifeLastAt=s.lifeLastAt||s.trainingLastAt||Date.now();state.inventory={captureBalls:s.inventory?.captureBalls??12,protein:s.inventory?.protein??6,healthy:s.inventory?.healthy??6,favorite:s.inventory?.favorite??6};state.eggs=(s.eggs||[]).map(e=>({...e,eggHolderId:e.eggHolderId||e.parentAId,readyAt:e.readyAt||Date.now()+30000}));state.breeding=s.breeding||{parentA:null,parentB:null};state.evolutionCandidate=null;state.currentZone=ZONES[s.currentZone]?s.currentZone:'hub';}
function saveGame(show=true){state.saveVersion=6;state.lifeLastAt=Date.now();localStorage.setItem(saveKey,JSON.stringify({state,playerHp:playerData.hp}));if(show)msg('บันทึกเกม V6.8 แล้ว');}
function loadGame(){try{const raw=localStorage.getItem(saveKey);if(raw){const s=JSON.parse(raw);if(s?.state){migrateLoadedState(s.state);playerData.hp=s.playerHp||100;applyLifeSimulation(Date.now(),true);msg('โหลดข้อมูล V6.8 แล้ว');return;}}const v5=localStorage.getItem(oldV5Key);if(v5){const s=JSON.parse(v5);if(s?.state){migrateLoadedState(s.state);playerData.hp=s.playerHp||100;state.currentZone='hub';msg('ย้าย Save V5 → V6.8 สำเร็จ');saveGame(false);return;}}const v4=localStorage.getItem(oldV4Key);if(v4){const s=JSON.parse(v4);if(s?.state){migrateLoadedState(s.state);playerData.hp=s.playerHp||100;state.currentZone='hub';msg('ย้าย Save เก่า → V6.8 สำเร็จ');saveGame(false);}}}catch(e){console.warn('load failed',e);}}
...
const captureBtn=el('captureBtn');captureBtn.addEventListener('pointerdown',e=>{e.preventDefault();captureBtn.setPointerCapture?.(e.pointerId);beginCaptureAim();});captureBtn.addEventListener('pointerup',e=>{e.preventDefault();executeCaptureThrow();});captureBtn.addEventListener('pointercancel',()=>cancelCaptureAim());el('summonBtn').onclick=summonThrow;el('recallBtn').onclick=()=>recall(true);el('skill1Btn').onclick=()=>useSkill(0);el('skill2Btn').onclick=()=>useSkill(1);el('skill3Btn').onclick=()=>useSkill(2);el('saveBtn').onclick=()=>saveGame(true);
...
let last=performance.now(),targetTick=0,lifeTick=0,firstFrame=true;
function loop(now){
  try{const dt=Math.min(.033,(now-last)/1000);last=now;updatePlayer(dt);updateCamera(dt);updateWorldLabels();updateFloatingTexts(dt);updateProjectiles(dt);updateEffects(dt);updateGroundDecals(dt);updateCaptureAimVisual();updateOwned(dt);updateHubCompanion(dt);updateRanchVisuals(dt);ensureWildPopulation(dt);for(const w of [...wilds])updateWild(w,dt);ranchPad.ring.rotation.z+=dt*.2;breedingPad.ring.rotation.z-=dt*.16;incubator.rotation.y+=dt*.12;targetTick-=dt;lifeTick-=dt;if(lifeTick<=0){lifeTick=5;applyLifeSimulation(Date.now());}if(targetTick<=0){targetTick=.12;updateTarget();renderHUD();renderSkillButtons();updateNpcUI();if(!el('monsterManager').classList.contains('hidden'))renderManager();}renderer.render(scene,camera);if(firstFrame){firstFrame=false;if(startup){startup.classList.add('ok');setTimeout(()=>startup.remove(),450);}}requestAnimationFrame(loop);}catch(err){console.error(err);startupText('เกมหยุดทำงาน: '+err.message,'error');}
}
