...
25. VFX Priority & Concurrency System
26. VFX Quality Settings (Low/Medium/High)
27. Audio Cue Mapping (VFX → เสียง)
28. VFX Performance Budget ต่อเหตุการณ์
29. VFX State Machine
30. ลำดับการทำ (Phases)
31. ไฟล์ที่กระทบ
32. การตรวจรับ
33. ข้อควรระวัง
34. สรุป
...
### 25.1 Priority Levels

| Priority | VFX | เหตุผล |
|----------|-----|--------|
| P0 (สูงสุด) | Capture Sequence, Screen Flash, Damage Number | ห้ามตัด |
| P1 | Skill VFX (enemy/area), Hit Flash, Summon/Recall | สำคัญมาก |
| P2 | Skill VFX (self/buff), Training, Evolution | สำคัญปานกลาง |
| P3 | Feed/Care, Bond Up, Condition Bad | น้อย |
| P4 (ต่ำสุด) | Trail particle, Ambient | ตัดได้ |

### 25.2 Concurrency Limit

```js
const VFX_LIMITS = { maxConcurrentEffects: 80, maxParticles: 200, maxGroundDecals: 8, maxFloatingTexts: 12 };
function canSpawnVFX(priority) {
  if (priority === 'P0') return true;
  if (effects.length >= VFX_LIMITS.maxConcurrentEffects) {
    for (let i = effects.length - 1; i >= 0; i--) {
      if (effects[i].priority === 'P4' || effects[i].priority === 'P3') {
        releaseTransientEffect(effects[i]); effects.splice(i, 1); break;
      }
    }
  }
  return effects.length < VFX_LIMITS.maxConcurrentEffects;
}
```
...
|---------|-----|--------|------|
| Pool size | 80 | 140 | 200 |
| Particle/effect | 40% | 70% | 100% |
| Shadow | off | on (low) | on (high) |
| Ground Decal max | 4 | 6 | 8 |
| Trail particle | 1 | 2 | 3 |
| Capture tension | แสงเท่านั้น | แสง+UI | แสง+UI+particle |
| Emissive | 0.3x | 0.6x | 1.0x |

```js
function detectVFXQuality() {
...
| VFX | เสียง | Timing | Volume |
|-----|------|--------|--------|
| Skill enemy hit | hit_[type] | 0ms | 0.7 |
| Skill area | area_[type] | 0ms | 0.8 |
| Skill heal | heal | 0ms | 0.6 |
| Skill shield | shield | 0ms | 0.5 |
| Summon | summon | 550ms | 0.7 |
| Recall | recall | 0ms | 0.5 |
| Capture throw | throw | 0ms | 0.6 |
| Capture impact | capture_hit | 550ms | 0.8 |
| Capture tension | tension_tick | 800ms (loop) | 0.3→0.6 |
| Capture success | capture_success | 2500ms | 1.0 |
| Capture fail | capture_fail | 2500ms | 0.8 |
| Faint | faint | 0ms | 0.7 |
| Level Up | levelup | 0ms | 0.6 |
| Evolution | evolution | 0ms (loop 1.2s) | 0.5→0.8 |
| Egg hatch | hatch | 0ms | 0.7 |
| Feed | feed | 0ms | 0.4 |
| Training | training | 0ms | 0.4 |
| Mastery Up | mastery | 0ms | 0.6 |
| Bond Up | bond | 0ms | 0.3 |
| Player hurt | player_hurt | 0ms | 0.6 |
| Zone transition | zone_change | 0ms | 0.4 |

### 27.2 Procedural SFX (ไม่ต้องไฟล์)
...
function playTensionHeartbeat(t) {
  const interval = 0.6 - t * 0.3;
  if (Math.floor(tensionTime / interval) !== Math.floor((tensionTime - dt) / interval)) {
    const osc = audioCtx.createOscillator();
    const gain = audioCtx.createGain();
    osc.type = 'sine';
    osc.frequency.value = t < 0.5 ? 60 : 80;
...
| เหตุการณ์ | Particle | Triangle | Draw calls | Render | Logic |
|----------|---------|---------|-----------|--------|-------|
| Skill enemy | 19 | 228 | 3 | 0.3ms | 0.1ms |
| Skill area (3) | 56 | 672 | 7 | 0.8ms | 0.3ms |
| Skill self | 26 | 312 | 4 | 0.4ms | 0.1ms |
| Summon | 34 | 408 | 5 | 0.5ms | 0.2ms |
| Recall | 16 | 192 | 3 | 0.2ms | 0.1ms |
| Capture (all) | ~40 | ~480 | ~6 | 0.6ms | 0.5ms |
| Evolution | 36 | 432 | 5 | 0.5ms | 0.2ms |
| Faint | 12 | 144 | 2 | 0.2ms | 0.1ms |
| Training | 8 | 96 | 2 | 0.1ms | 0.05ms |

Worst case (area + capture + wild): 104 particle, 1248 tri, 18 draw calls, ~2.5ms — ภายใน 16ms budget
...
```
IDLE → ACTIVE → FADING → ENDED → RELEASED
                ↓ (life=0)
              ENDED → RELEASED (pool.release หรือ dispose)
```

| Kind | Spawn | Active | End | Release |
|------|-------|--------|-----|---------|
| spark | acquire | move+spin | life=0 | pool |
| ring | new Mesh | scale↑ | life=0 | dispose |
| ground-decal | new Group | spin | life=0 | dispose |
| damage-number | DOM | position | life=0 | DOM remove |
| capture-seq | new ball | phase machine | result | dispose |
| screen-flash | DOM | opacity | timeout | DOM hidden |
| hit-flash | mat change | — | timeout | mat restore |
| summon-beam | new Mesh | scale↑ | life=0 | dispose |
| shield-aura | new Mesh | pulse | duration=0 | dispose |
| area-wave | new Mesh | scale↑↑ | life=0 | dispose |

```js
function updateAllVFX(dt) {
  updateCaptureSequence(dt);  // P0 ก่อน
  updateEffects(dt);          // spark/ring/beam/aura/wave
  updateGroundDecals(dt);     // วงพื้น
  updateFloatingTexts(dt);    // DOM
  updateProjectiles(dt);      // กระสุน
  // camera shake
  if (cameraShake.time > 0) {
    cameraShake.time -= dt;
    const t = Math.max(0, cameraShake.time / cameraShake.duration);
    const mag = cameraShake.mag * t;
