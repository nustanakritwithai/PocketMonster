...
const completeStageClear=js.match(/function completeStageClear\(stageId,\{recovered=false\}=\{\}\)\{[\s\S]*?\n\}/)?.[0]||'';
assert.match(completeStageClear,/state\.stageProgress=next;\s*try\{saveGame\(false\);\}catch\{\}[\s\S]*?runBestEffortCombatPresentation\(\(\)=>\{renderStageSelect\(\);renderStageReward/,'Boss clear persists progression before refreshing stage metadata and rewards best-effort');
assert.doesNotMatch(completeStageClear,/renderWarpPrompt/,'Boss clear must not call the removed in-scene warp prompt renderer');
