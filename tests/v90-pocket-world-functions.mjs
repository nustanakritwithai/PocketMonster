...
assert.match(preload, /chat-runtime\.mjs\?v=8\.4\.0-smooth-presence-1/, 'live preload cache-busts the top-right chat');
assert.match(preload, /chat-runtime\.mjs\?v=8\.4\.0-smooth-presence-1[\s\S]*game\-v800\.js\?v=835/, 'legacy preload binds chat before game overlays');
...
assert.match(shellJs, /await import\('\.\/chat-runtime\.mjs\?v=8\.4\.0-smooth-presence-1'\)/, 'persistent V9 shell owns the one presence-aware chat transport');
