...
assert.match(preload, /chat-runtime\.mjs\?v=8\.4\.0-central-hp-1/, 'live preload cache-busts the top-right chat');
assert.match(preload, /chat-runtime\.mjs\?v=8\.4\.0-central-hp-1[\s\S]*game\-v800\.js\?v=839/, 'legacy preload binds chat before game overlays');
...
assert.match(shellJs, /await import\('\.\/chat-runtime\.mjs\?v=8\.4\.0-central-hp-1'\)/, 'persistent V9 shell owns the one presence-aware chat transport');
...
assert.match(livingJs, /from '\.\/world-presence-v800\.mjs\?v=6'/, 'living world uses the cache-busted shared presence helper');
...
assert.doesNotMatch(html, /id="controlPanelSwitcher"/, 'V9 no longer ships the human/throw switcher');
...

{
  ...
  assert.deepEqual(window.POCKETMONSTER_WORLD_STATE(), { zone: 'pirate-fruit', x: 1.5, z: -2, dir: 0.4, locomotion: 'idle', animation: null });
