...
assert.match(preload, /chat-runtime\.mjs\?v=8\.4\.0-original-world-1[\s\S]*game\-v800\.js\?v=839/, 'legacy preload binds chat before game overlays');
...
assert.match(shellJs, /await import\('\.\/chat-runtime\.mjs\?v=8\.4\.0-original-world-1'\)/, 'persistent V9 shell owns the one presence-aware chat transport');
...
assert.match(livingJs, /from '\.\/world-presence-v800\.mjs\?v=7'/, 'living world uses the cache-busted shared presence helper');
