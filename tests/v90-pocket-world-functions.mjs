...
assert.match(preload, /chat-runtime\.mjs\?v=8\.4\.0-full-presentation-1/, 'live preload cache-busts the top-right chat');
assert.match(preload, /chat-runtime\.mjs\?v=8\.4\.0-full-presentation-1[\s\S]*game-v800\.js\?v=819/, 'legacy preload binds chat before game overlays');
...
assert.match(shellJs, /await import\('\.\/chat-runtime\.mjs\?v=8\.4\.0-full-presentation-1'\)/, 'persistent V9 shell owns the one presence-aware chat transport');
