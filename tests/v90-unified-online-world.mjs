...
assert.match(indexHtml, /entry-preload-v900\.mjs\?v=976/, 'active HTML cache-busts the unified ship-control entry');
...
assert.match(entry, /await prepareLaunch\(config\)[\s\S]*await import\('\.\/online-world-shell-v900\.mjs\?v=68'\)/, 'top-level authenticates once before starting the cache-busted shell');
...
const parentShellIndex = entry.indexOf("await import('./online-world-shell-v900.mjs?v=68')");
...
assert.match(shell, /searchParams\.set\('shellRevision', '69'\)/, 'persistent shell cache-busts the changed scene HTML');
