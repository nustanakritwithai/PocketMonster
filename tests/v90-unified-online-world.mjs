...
assert.match(indexHtml, /entry\-preload\-v900\.mjs\?v=1003/, 'active HTML cache-busts the unified ship-control entry');
...
assert.match(entry, /await prepareLaunch\(config\)[\s\S]*await import\('\.\/online\-world\-shell\-v900\.mjs\?v=94'\)/, 'top-level authenticates once before starting the cache-busted shell');
...
const parentShellIndex = entry.indexOf("await import('./online-world-shell-v900.mjs?v=94')");
...
assert.match(pirateOfflineHtml, /persistent-fullscreen-v900\.mjs\?v=4[\s\S]*pocket-bootstrap\.mjs\?v=10/, 'Pirate iframe installs the fullscreen bridge before its save bootstrap');
...
assert.match(shell, /combined-worlds-v900\.mjs\?v=973/, 'persistent shell cache-busts the changed world catalog');
assert.match(shell, /searchParams\.set\('shellRevision', '92'\)/, 'persistent shell cache-busts the changed scene HTML');
assert.match(worlds, /combined-worlds-v900\.mjs\?v=973/, 'scene router cache-busts the changed world catalog');
assert.match(sceneEntry, /worlds-v900\.mjs\?v=977/, 'hosted scene cache-busts the three-world router');
...
const childWorldImportIndex = sceneEntry.indexOf("await import('./worlds-v900.mjs?v=977')");
