...
assert.match(indexHtml, /entry\-preload\-v900\.mjs\?v=990/, 'active HTML cache-busts the unified ship-control entry');
...
assert.match(entry, /await prepareLaunch\(config\)[\s\S]*await import\('\.\/online\-world\-shell\-v900\.mjs\?v=81'\)/, 'top-level authenticates once before starting the cache-busted shell');
...
const parentShellIndex = entry.indexOf("await import('./online-world-shell-v900.mjs?v=81')");
...
assert.match(shell, /searchParams\.set\('shellRevision', '80'\)/, 'persistent shell cache-busts the changed scene HTML');
