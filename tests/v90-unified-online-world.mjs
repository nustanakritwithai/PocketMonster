...
assert.match(indexHtml, /entry-preload-v900\.mjs\?v=941/, 'active HTML cache-busts the restored minimap entry');
assert.match(indexHtml, /style-v900\.css\?v=939/, 'active HTML cache-busts the Pirate-primary presentation and persistent shell layout');
assert.match(sceneHtml, /style-v900\.css\?v=939/, 'hosted scene cache-busts the same V9 stylesheet');
...
assert.match(entry, /await prepareLaunch\(config\)[\s\S]*await import\('\.\/online-world-shell-v900\.mjs\?v=31'\)/, 'top-level authenticates once before starting the cache-busted shell');
...
const parentShellIndex = entry.indexOf("await import('./online-world-shell-v900.mjs?v=31')");
...
assert.match(shell, /searchParams\.set\('shellRevision', '37'\)/, 'persistent shell cache-busts the changed scene HTML');
