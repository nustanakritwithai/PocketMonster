...
assert.match(indexHtml, /entry-preload-v900\.mjs\?v=933/, 'active HTML cache-busts the restored minimap entry');
assert.match(indexHtml, /style-v900\.css\?v=935/, 'active HTML cache-busts the Pirate-primary presentation and persistent shell layout');
assert.match(sceneHtml, /style-v900\.css\?v=935/, 'hosted scene cache-busts the same V9 stylesheet');
...
assert.match(entry, /await prepareLaunch\(config\)[\s\S]*await import\('\.\/online-world-shell-v900\.mjs\?v=23'\)/, 'top-level authenticates once before starting the cache-busted shell');
...
const parentShellIndex = entry.indexOf("await import('./online-world-shell-v900.mjs?v=23')");
...
assert.match(shell, /searchParams\.set\('shellRevision', '29'\)/, 'persistent shell cache-busts the changed scene HTML');
assert.match(worlds, /combined-worlds-v900\.mjs\?v=921/, 'scene router cache-busts the changed world catalog');
assert.match(sceneEntry, /worlds-v900\.mjs\?v=926/, 'hosted scene cache-busts the three-world router');
...
const childWorldImportIndex = sceneEntry.indexOf("await import('./worlds-v900.mjs?v=926')");
