...
assert.match(indexHtml, /entry-preload-v900\.mjs\?v=934/, 'active HTML cache-busts the restored minimap entry');
...
assert.match(entry, /await prepareLaunch\(config\)[\s\S]*await import\('\.\/online-world-shell-v900\.mjs\?v=24'\)/, 'top-level authenticates once before starting the cache-busted shell');
...
const parentShellIndex = entry.indexOf("await import('./online-world-shell-v900.mjs?v=24')");
...
assert.match(shell, /searchParams\.set\('shellRevision', '30'\)/, 'persistent shell cache-busts the changed scene HTML');
