...
assert.match(indexHtml, /entry-preload-v900\.mjs\?v=966/, 'active HTML cache-busts the restored minimap entry');
...
assert.match(entry, /await prepareLaunch\(config\)[\s\S]*await import\('\.\/online-world-shell-v900\.mjs\?v=57'\)/, 'top-level authenticates once before starting the cache-busted shell');
...
const parentShellIndex = entry.indexOf("await import('./online-world-shell-v900.mjs?v=57')");
...
assert.match(pirateBootstrap, /await installPirateSaveSandbox\(\);[\s\S]*await import\('\.\/assets\/index-[A-Za-z0-9_-]+\.js'\)/, 'Pirate save hydration completes before the vendored runtime loads');
