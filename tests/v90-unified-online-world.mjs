...
assert.match(shell, /combined-worlds-v900\.mjs\?v=928/, 'persistent shell cache-busts the changed world catalog');
assert.match(shell, /searchParams\.set\('shellRevision', '40'\)/, 'persistent shell cache-busts the changed scene HTML');
assert.match(worlds, /combined-worlds-v900\.mjs\?v=928/, 'scene router cache-busts the changed world catalog');
assert.match(sceneEntry, /worlds-v900\.mjs\?v=933/, 'hosted scene cache-busts the three-world router');
...
const childWorldImportIndex = sceneEntry.indexOf("await import('./worlds-v900.mjs?v=933')");
