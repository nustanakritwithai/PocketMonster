...
assert.match(indexHtml, /entry-preload-v900\.mjs\?v=926/, 'active HTML cache-busts the persistent-shell entry');
...
assert.match(entry, /await prepareLaunch\(config\)[\s\S]*await import\('\.\/online-world-shell-v900\.mjs\?v=17'\)/, 'top-level authenticates once before starting the cache-busted shell');
...
const parentShellIndex = entry.indexOf("await import('./online-world-shell-v900.mjs?v=17')");
...
assert.match(shell, /searchParams\.set\('shellRevision', '15'\)/, 'persistent shell cache-busts the changed scene HTML');
