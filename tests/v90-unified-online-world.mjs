...
assert.match(indexHtml, /entry-preload-v900\.mjs\?v=915/, 'active HTML cache-busts the persistent-shell entry');
...
assert.match(entry, /await prepareLaunch\(config\)[\s\S]*await import\('\.\/online-world-shell-v900\.mjs\?v=5'\)/, 'top-level authenticates once before starting the shell');
...
const parentShellIndex = entry.indexOf("await import('./online-world-shell-v900.mjs?v=5')");
...
assert.match(sceneEntry, /v900\.html\?v=911/, 'hosted scene cache-busts the shared V9 DOM template');
...
assert.match(sceneEntry, /worlds-v900\.mjs\?v=913/, 'hosted scene boots the existing three-world router');
const childWorldImportIndex = sceneEntry.indexOf("await import('./worlds-v900.mjs?v=913')");
