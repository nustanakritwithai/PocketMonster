...
assert.match(indexHtml, /entry-preload-v900\.mjs\?v=970/, 'active HTML cache-busts the unified ship-control entry');
assert.match(indexHtml, /style-v900\.css\?v=968/, 'active HTML cache-busts the centered Pirate helm styling');
assert.match(sceneHtml, /style-v900\.css\?v=968/, 'hosted scene cache-busts the same V9 stylesheet');
...
assert.match(entry, /await prepareLaunch\(config\)[\s\S]*await import\('\.\/online-world-shell-v900\.mjs\?v=62'\)/, 'top-level authenticates once before starting the cache-busted shell');
...
const parentShellIndex = entry.indexOf("await import('./online-world-shell-v900.mjs?v=62')");
...
assert.match(sceneEntry, /v900\.html\?v=918/, 'hosted scene cache-busts the shared V9 DOM template');
...
assert.match(shell, /combined-worlds-v900\.mjs\?v=952/, 'persistent shell cache-busts the changed world catalog');
assert.match(shell, /searchParams\.set\('shellRevision', '65'\)/, 'persistent shell cache-busts the changed scene HTML');
assert.match(worlds, /combined-worlds-v900\.mjs\?v=952/, 'scene router cache-busts the changed world catalog');
assert.match(sceneEntry, /worlds-v900\.mjs\?v=957/, 'hosted scene cache-busts the three-world router');
...
const childWorldImportIndex = sceneEntry.indexOf("await import('./worlds-v900.mjs?v=957')");
