...
assert.match(indexHtml, /entry-preload-v900\.mjs\?v=987/, 'active HTML cache-busts the unified ship-control entry');
...
assert.match(entry, /await prepareLaunch\(config\)[\s\S]*await import\('\.\/online-world-shell-v900\.mjs\?v=78'\)/, 'top-level authenticates once before starting the cache-busted shell');
...
const parentShellIndex = entry.indexOf("await import('./online-world-shell-v900.mjs?v=78')");
...
assert.match(shell, /searchParams\.set\('shellRevision', '77'\)/, 'persistent shell cache-busts the changed scene HTML');
