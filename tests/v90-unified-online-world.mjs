...
assert.match(indexHtml, /entry-preload-v900\.mjs\?v=967/, 'active HTML cache-busts the restored offline NPC menu entry');
assert.match(indexHtml, /style-v900\.css\?v=967/, 'active HTML cache-busts the restored offline NPC menu styling');
assert.match(sceneHtml, /style-v900\.css\?v=967/, 'hosted scene cache-busts the same V9 stylesheet');
...
assert.match(entry, /await prepareLaunch\(config\)[\s\S]*await import\('\.\/online-world-shell-v900\.mjs\?v=59'\)/, 'top-level authenticates once before starting the cache-busted shell');
...
const parentShellIndex = entry.indexOf("await import('./online-world-shell-v900.mjs?v=59')");
...
assert.match(sceneEntry, /v900\.html\?v=917/, 'hosted scene cache-busts the shared V9 DOM template');
...
assert.match(shell, /combined-worlds-v900\.mjs\?v=949/, 'persistent shell cache-busts the changed world catalog');
assert.match(shell, /searchParams\.set\('shellRevision', '62'\)/, 'persistent shell cache-busts the changed scene HTML');
assert.match(worlds, /combined-worlds-v900\.mjs\?v=949/, 'scene router cache-busts the changed world catalog');
assert.match(sceneEntry, /worlds-v900\.mjs\?v=954/, 'hosted scene cache-busts the three-world router');
...
const childWorldImportIndex = sceneEntry.indexOf("await import('./worlds-v900.mjs?v=954')");
