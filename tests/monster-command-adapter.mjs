...
assert.equal(sanitizeMonsterCommand({ ...base, kind: 'recall', expectedActiveGeneration: 1 }).kind, 'recall', 'recall command requires the active generation');
assert.equal(sanitizeMonsterCommand({ ...base, kind: 'switch', expectedActiveInstanceId: 'monster-0', expectedActiveGeneration: 1, targetPoint: { x: 1, y: 0, z: 2 } }).kind, 'switch', 'switch requires expected active owner and target point');
assert.equal(sanitizeMonsterCommand({ ...base, kind: 'switch', targetPoint: { x: 1, y: 0, z: 2 } }), null, 'switch requires expected active instance');
