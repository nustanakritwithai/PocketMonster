...
assert.match(legacyHtml, /entry-preload\.mjs\?v=817/, 'legacy V8 entry must cache-bust the updated game preload');
const preload = fs.readFileSync(new URL('entry-preload.mjs', root), 'utf8');
assert.match(preload, /await prepareLaunch/);
assert.match(preload, /await import\('\.\/game-v800\.js\?v=817'\)/, 'preload must import game only after launch gate');
