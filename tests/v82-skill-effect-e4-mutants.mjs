...
const liveMutations = [
  ...
  ['drop mobility application', 'applyPlannedMobilityEffects(a,move,planned.movementResult,planned.displacementResults,targets,targetDamageCommitted,actualHitCount)', '0'],
  ...
  ['nearest damage finalizes before accepted skill transaction', 'damageWild(target,plannedRes.damage,{type:move.type,eff:plannedRes.eff,deferDefeat:true,commitReceipt:damageReceipt});\n      if(damageReceipt.committed)', 'damageWild(target,plannedRes.damage,{type:move.type,eff:plannedRes.eff,deferDefeat:false,commitReceipt:damageReceipt});\n      if(damageReceipt.committed)'],
  ['area damage finalizes before accepted skill transaction', 'damageWild(target,plannedRes.damage,{type:move.type,eff:plannedRes.eff,deferDefeat:true,commitReceipt:damageReceipt});\n      if(!damageReceipt.committed)', 'damageWild(target,plannedRes.damage,{type:move.type,eff:plannedRes.eff,deferDefeat:false,commitReceipt:damageReceipt});\n      if(!damageReceipt.committed)'],
  ['finalize defeat before accepted receipt and rewards commit', 'receipt=Object.freeze({', 'settleCommittedSkillBattleTransaction(targets,targetDamageCommitted,targetCommittedDamage,move.power||10,a.inst.instanceId,contributionEvents);receipt=Object.freeze({'],
  ['drop accepted skill defeat finalization', 'settleCommittedSkillBattleTransaction(targets,targetDamageCommitted,targetCommittedDamage,move.power||10,a.inst.instanceId,contributionEvents);', 'void targetDamageCommitted;'],
  ['reverse multi-target defeat finalization', 'pending.sort((left,right)=>stableCombatIdCompare(left.target.id,right.target.id)||left.index-right.index);', 'pending.sort((left,right)=>stableCombatIdCompare(right.target.id,left.target.id)||right.index-left.index);'],
  ['restore locale-sensitive defeat ordering', 'stableCombatIdCompare(left.target.id,right.target.id)', 'String(left.target.id).localeCompare(String(right.target.id))'],
  ['finalize an uncommitted stale target', 'targetDamageCommitted[index]===true&&target&&!target.dead&&target.hp<=0', 'target&&!target.dead&&target.hp<=0'],
  ['first defeat failure strands later lethal targets', 'catch(error){if(!hasFailure){firstFailure=error;hasFailure=true;}}', 'catch(error){throw error;}'],
  ['swallow deferred defeat failure', 'if(hasFailure)throw firstFailure;', 'void firstFailure;'],
  ['disable actor movement', 'if(movementResult?.applied&&actualHitCount>0)', 'if(false)'],
  ['replay stale actor movement after rejected damage', 'if(movementResult?.applied&&actualHitCount>0)', 'if(movementResult?.applied)'],
  ['keep stale AI decision', '    a.aiDecision=null;\n    appliedCount++;', '    void a.aiDecision;\n    appliedCount++;'],
  ...
  ['replay stale displacement after reset', 'targetDamageCommitted[index]!==true||!result?.applied', '!result?.applied'],
  ['move pending-defeat target', 'target.dead||!(target.hp>0)||!target.mesh?.position', 'target.dead||!target.mesh?.position'],
