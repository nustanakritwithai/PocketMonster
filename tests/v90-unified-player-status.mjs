...
assert.match(css, /\.mmorpg-player-body\{[^}]*flex-direction:row/, 'name and tubes share one horizontal row');
assert.match(css, /\.mmorpg-player-vitals\{[^}]*flex-direction:row/, 'HP MP energy sit after the name as long tubes');
assert.match(css, /\.mmorpg-player-vital\{[^}]*flex:1 1 0/, 'each tube grows along the row instead of stacking');
