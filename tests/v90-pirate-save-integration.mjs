...
assert.match(boot, /index\.html\?v=921/, 'parent cache-busts the Pirate child HTML without the circular minimap');
assert.match(combined, /boot-pirate-fruit-v900\.mjs\?v=929/, 'world catalog keeps the current Pirate boot module revision');
assert.match(entry, /online-world-shell-v900\.mjs\?v=39/, 'top-level entry cache-busts the production Combat transport and BFCache restore behavior');
assert.match(sceneHtml, /scene-entry-v900\.mjs\?v=36/, 'scene HTML cache-busts the shared transport dependency chain and fullscreen ownership');
assert.match(sceneHtml, /style-v900\.css\?v=957/, 'scene HTML cache-busts the persistent fullscreen control layout');
assert.match(offlineHtml, /pocket-presentation\.mjs\?v=12/, 'offline HTML cache-busts presentation integration after child minimap retirement');
