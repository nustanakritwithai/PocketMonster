...
assert.match(boot, /pirate-fruit-control-hud-v900\.mjs\?v=3/, 'parent boot cache-busts the child HUD retirement contract');
...
assert.match(entry, /online-world-shell-v900\.mjs\?v=25/, 'top-level entry cache-busts the production Combat transport and BFCache restore behavior');
assert.match(sceneHtml, /scene-entry-v900\.mjs\?v=29/, 'scene HTML cache-busts the shared transport dependency chain and fullscreen ownership');
...
assert.match(presentation, /pirate-fruit-control-hud-v900\.mjs\?v=3/, 'presentation loads the HUD policy that retires the circular child minimap');
