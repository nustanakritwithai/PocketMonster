...
assert.match(combined, /boot-pirate-fruit-v900.mjs\?v=959/, 'world catalog keeps the current Pirate boot module revision');
assert.match(entry, /online-world-shell-v900\.mjs\?v=72/, 'top-level entry cache-busts the unified ship-control shell');
assert.match(sceneHtml, /scene-entry-v900\.mjs\?v=64/, 'scene HTML cache-busts the unified Pirate ship-control bridge');
assert.match(sceneHtml, /style-v900\.css\?v=978/, 'scene HTML cache-busts the helm placement beside chat');
...
assert.match(presentation, /pirate-fruit-control-hud-v900\.mjs\?v=22/, 'presentation loads the HUD policy that retires the helm center-panel duplicate');
