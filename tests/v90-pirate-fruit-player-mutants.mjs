...
assert.equal(worldById('pocket-monster').runtime, './game-v800.js?v=839', 'mutant 0e: original game runtime is game-v800.js');
assert.equal(worldById('pirate-fruit').runtime, './boot-pirate-fruit-v900.mjs?v=973', 'mutant 0e2: pirate world still boots through the cache-busted pirate boot module');
...
assert.match(boot, /PIRATE_FRUIT_OFFLINE_ENTRY/, 'mutant 3: pirate world must load the canonical local Pirate client');
assert.match(boot, /remote: false/, 'mutant 4: canonical Pirate route must not load a remote host');
...
assert.match(boot, /game\-v800\.js\?v=839&animalControl=pirate-fruit/, 'mutant 23d: throw boots a dedicated animal-control instance');
...
assert.match(boot, /source: 'pirate-fruit-offline'/, 'mutant 24: pirate human panel is the canonical local Pirate client');
...
assert.match(shellJs, /chat-runtime\.mjs\?v=8\.4\.0-original-world-1/, 'mutant 27: persistent V9 shell loads one presence-aware Pocket chat transport');
