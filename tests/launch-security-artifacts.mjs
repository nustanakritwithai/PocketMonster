...
for (const name of ['index.html', 'v800.html']) {
  ...
  const entryName = name === 'index.html' ? 'entry-preload-v900.mjs' : 'entry-preload.mjs';
  const inline = html.match(/<script>([\s\S]*?)<\/script>/)?.[1];
  assert.ok(inline, `${name} must have an early inline launch scrubber`);
  assert.ok(html.indexOf(`<script>${inline}</script>`) < html.indexOf(entryName), `${name} must scrub the handoff before loading modules`);
...
const combined = [bootstrap, preload, preloadV900, read('firebase-launcher-entry.mjs'), read('server-auth.mjs')].join('\n');
