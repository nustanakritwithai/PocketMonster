...
for (const label of ['HP', 'ATK', 'DEF', 'SPD', 'CR', 'Condition']) {
  assert.match(js, new RegExp(`character-status-${label.toLowerCase()}`), `Status/Overview must render ${label}`);
}
assert.match(js, /renderFullCharacterStatus\(\)/, 'manager must invoke Status/Overview rendering');
assert.doesNotMatch(js, /state\.ui\.(?:hp|maxHp|atk|def|spd|cr|condition)\s*=/, 'Status/Overview must not duplicate gameplay data into state.ui');
