...
const requiredDecls=["const ranchVisuals=new Map()","const joy={x:0,y:0}","const unifiedMobileControls=window.POCKETMONSTER_UNIFIED_MOBILE_CONTROLS"];
for(const d of requiredDecls){assert.ok(js.includes(d),`missing declaration ${d}`);}
assert.ok(js.includes("unifiedMobileControls.registerAdapter('pocket-monster'"),'Pocket input adapter not registered');
assert.ok(unifiedControls.includes("getElementById?.('joystick')")&&unifiedControls.includes("getElementById?.('cameraPad')"),'unified joystick/camera bindings missing');
