...
const block=()=>js.match(/['"]poison-marsh['"]\s*:\s*\{[\s\S]*?(?=\n  ['"]sky-ruins['"]\s*:)/)?.[0]||'';
...
assert.match(block(),/stageId:'poison-marsh'/,'Poison Marsh stays catalog-linked');
...
assert.equal(STAGE_BY_ID['poison-marsh'].capturePolicy,'normal-wild-only','Boss capture policy stays disabled');
...
const unsafe=`${block()}\nplayerData.hp-=1`;
assert.throws(()=>assert.doesNotMatch(unsafe,/playerData\.hp|status|damage|teleport|fly|jump/),'mutation 5: terrain/combat logic is rejected');
console.log('V8 Poison Marsh mutation guards: PASS (5/5 killed)');
