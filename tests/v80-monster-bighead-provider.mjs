...
assert.doesNotMatch(js, /createBigheadMonsterProvider/, 'Phase 1 must not wire the monster provider into the game');
assert.doesNotMatch(js, /procedural-bighead-monster/, 'gameplay must not import the Phase 1 provider');
assert.match(js, /function makeSpeciesMesh\(/, 'legacy mesh builders stay as Phase 3 fallback');
