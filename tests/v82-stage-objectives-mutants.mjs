...
function assertRuntimeContract(game,html,css=fs.readFileSync(new URL('../style-v800.css',import.meta.url),'utf8')){
  ...
  assert.match(css,/\.starter-journey,\.quest-tracker\{[^}]*top:calc\(var\(--safe-top\) \+ var\(--touch-min\) \+ 12px\)/);
...
const runtimeMutants=[
  ...
  ['restore centered objective banner',gameSource,htmlSource,fs.readFileSync(new URL('../style-v800.css',import.meta.url),'utf8').replace('top:calc(var(--safe-top) + var(--touch-min) + 12px);left:var(--safe-left);transform:none','top:132px;left:50%;transform:translateX(-50%)')],
  ['keep the oversized tracker under the top menu',gameSource,htmlSource,fs.readFileSync(new URL('../style-v800.css',import.meta.url),'utf8').replace('top:calc(var(--safe-top) + var(--touch-min) + 12px)','top:54px').replace('width:min(156px,24vw)','width:min(212px,36vw)')],
