...
assert.match(schema, /export const ASSET_REVISION = '813'/, 'mutant 2: atmosphere uses the current ASSET_REVISION');
...
assert.match(ground, /scene\.fog\.near=zone==='cave'\?15:zone==='frozen-pass'\?18:zone==='rocky-canyon'\?24:zone==='sky-ruins'\?22:zone==='poison-marsh'\?18:zone==='dream-shrine'\?20:zone==='haunted-woods'\?14:zone==='shadow-city'\?16:zone==='steel-factory'\?18:30/, 'mutant 5: stage fog near distances stay intentional');
assert.match(ground, /scene\.fog\.far=zone==='cave'\?50:zone==='frozen-pass'\?62:zone==='rocky-canyon'\?68:zone==='sky-ruins'\?80:zone==='poison-marsh'\?58:zone==='dream-shrine'\?64:zone==='haunted-woods'\?48:zone==='shadow-city'\?60:zone==='steel-factory'\?66:76/, 'mutant 6: stage fog far distances stay intentional');
...
assert.match(painterSrc, /zoneType === 'woods'/, 'mutant 21: Haunted Woods grid still uses the slate speckle path');
assert.match(painterSrc, /zoneType === 'city'/, 'mutant 22: Shadow City grid still uses the cyan speckle path');
assert.match(painterSrc, /zoneType === 'factory'/, 'mutant 23: Steel Factory grid still uses the metallic speckle path');
assert.match(extractFn('switchZone'), /setZoneGround\(zone\)/, 'mutant 17: zone changes still call setZoneGround');
assert.match(js, /sphereGeometry\(\.16\*scale,12,10\)/, 'mutant 24: monster shine stays a sphere');
