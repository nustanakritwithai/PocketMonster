...
assert.match(schema, /export const ASSET_REVISION = '800'/, 'mutant 2: atmosphere does not bump ASSET_REVISION');
...
assert.match(js, /makePad\(7,3,3\.4,0x22c55e,\.17\)/, 'mutant 13: ranch pad call stays');
assert.match(js, /function spawnRingPulse[\s\S]*?boxGeometry\(size,\.02,size\)/, 'mutant 14: Phase 5 does not revert ring pulses');
assert.doesNotMatch(flower, /emissiveIntensity/, 'mutant 15: flower emissive glow stays for Phase 6');
