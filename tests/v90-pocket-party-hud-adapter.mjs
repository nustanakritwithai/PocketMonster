...
assert.match(game, /function selectPartySlotCommand\(slot\)\{[\s\S]{0,700}switchPartySlot\(index\)/, 'slot selection routes through the existing gameplay switch exactly once');
assert.match(game, /function selectPartySlotCommand\(slot\)\{[\s\S]{0,700}characterUI\.requestSwitchParty\(index\)/, 'slot selection respects the character controller switch gate');
