...
const liveMutants = [
  ...
  ['bypass live effect readiness', source => replaceOnce(source, 'canApply:command=>canApplyLiveSkill(command)', 'canApply:()=>true')],
  ...
  ['activate deferred Self effect', source => replaceOnce(source, "command.targetKind==='NearestEnemy'||command.targetKind==='EnemyArea'", "command.targetKind==='NearestEnemy'||command.targetKind==='EnemyArea'||command.targetKind==='Self'")],
  ...
  ['silently broaden deferred manual effects', source => replaceOnce(source, 'return Boolean(definition?.directDamage', 'return Boolean(true')],
