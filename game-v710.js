...
console.info(`Monster Life RPG V7.1.0 • Balance Foundation engine v${BALANCE_SCHEMA_VERSION} loaded`);
...
startupText('กำลังสร้าง Monster Life RPG V7.1.0…');
...
const joyEl=el('joystick'); if(!joyEl) throw new Error('V7.1.0 boot: #joystick not found');
const stick=el('stick'); if(!stick) throw new Error('V7.1.0 boot: #stick not found');
...
function updateWild(w,dt,canEngage=false){
  ...
  if(distance>1.25){
    ...
    if(activeSummon&&activeSummon.mesh){
      ...
      spawnDamageNumber(playerDmg,safeVec3(player.position).add(new THREE.Vector3(0,1.45,0)),{type:wildTypes(w)[0]});
      triggerCameraShake(w.boss?.17:.1,w.boss?.22:.15);
      setHumanoidAction(player,'hurt',.24);
...
function evolveMonster(id,pathId){const inst=getInst(id),sp=spById[inst?.speciesId],path=sp?.evolutionPaths?.find(p=>p.id===pathId);if(!inst||!path||inst.evolutionPath)return;const st=evoRequirementStatus(inst,path);if(!st.ok){msg('ยังไม่ผ่านเงื่อนไข Evolution');return;}if(!confirm(`Evolution ย้อนกลับไม่ได้\n${displayName(inst)} → ${path.name}\nยืนยันหรือไม่?`))return;inst.evolutionPath=path.id;if(path.secondaryType&&sp.allowedSecondary.includes(path.secondaryType))inst.secondaryType=path.secondaryType;
  // V7.7: Track evolution history (identity-preserving, R10)
  if(!Array.isArray(inst.evolutionHistory))inst.evolutionHistory=[];
  inst.evolutionHistory.push({from:inst.formId||null,to:path.id,evolutionId:pathId,at:Date.now()});
  if(!inst.formId)inst.formId=path.id;
  appendHistory(inst,{type:'evolution',from:inst.speciesId,to:path.id});
...
function migrateLoadedState(s){
  ...
  state.collection=clean.collection.map(ensureInstanceShape);
...
function saveGame(show=true){
  ...
  if(show)msg('บันทึกเกม V7.1.0 แล้ว');
}
function loadGame(){
  try{
    ...
    else if(saved.source==='current')msg('โหลดข้อมูล V7.1.0 แล้ว');
    else msg('ย้าย Save เก่า → V7.1.0 สำเร็จ');
