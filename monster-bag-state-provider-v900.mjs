import { loadServerSave } from './server-player-data.mjs';
...
export function createMonsterBagStateProvider({ config, sessionToken, getSessionToken = null,
  load = loadServerSave, fetchImpl = globalThis.fetch, clientVersion = '8.4.0' } = {}) {
  ...
  const refresh = () => {
    ...
    const work = (async () => {
      try {
        const loaded = await load(config, requestToken);
        ...
        if (!loaded?.envelope?.state || !Number.isSafeInteger(loaded.revision) || loaded.revision < 0) {
          current = empty('SAVE_NOT_FOUND'); return { ok: false, code: current.code };
        }
        const envelope = structuredClone(loaded.envelope);
        current = Object.freeze({ available: true, revision: loaded.revision, envelope,
          slots: resolveMonsterPartySlots(envelope.state), code: null });
  ...
  const assignToSlot = (instanceId, slot) => {
    const requestToken = bindSession();
    ...
    if (typeof instanceId !== 'string' || !/^[A-Za-z0-9._:-]{1,96}$/.test(instanceId) || !Number.isInteger(slot) || slot < 0 || slot > 2)
    ...
    const work = (async () => {
      try {
        ...
        if (current.slots[slot]?.instanceId === instanceId) return { ok: true, state: current, revision: current.revision };
        const retryKey = `${current.revision}:${instanceId}:${slot}`;
        // เก็บ UUID เดิมสำหรับ retry; ห้ามใส่ session token ลง commandId
        const command = retries.get(retryKey) || Object.freeze({ instanceId, slot,
        ...
        try {
          response = await fetchImpl(new URL('api/monsters/party-slot', `${config.apiBaseUrl.replace(/\/$/, '')}/`).href, {
        ...
        const result = await refresh();
        if (stale(requestToken, requestEpoch)) return { ok: false, code: 'STALE_SESSION' };
        return result.ok && result.state.revision >= payload.revision && result.state.slots[slot]?.instanceId === instanceId
  ...
  return Object.freeze({ snapshot() { bindSession(); return current; }, refresh, assignToSlot,
    dispose() { disposed = true; epoch += 1; current = empty('SESSION_UNAVAILABLE'); retries.clear(); for (const request of requests) request.abort(); },
  });
}
