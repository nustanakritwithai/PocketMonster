...
function useSkill(index){
  ...
  if(move.targetType==='enemy'){
    ...
    triggerMonsterAction(a.mesh,'attack',0.22);spawnElementalFX(move.type,a.mesh.position.clone().add(new THREE.Vector3(0,.6,0)),'summon',0.8);spawnSkillSprite(move.type,a.mesh.position.clone().add(new THREE.Vector3(0,.6,0)),0.8,0.6);spawnGroundDecal(move.type,a.mesh.position.clone(),{radius:1.2,duration:1.2,intensity:.95});
...
function renderRanchStoragePage(){const roster=el('ranchStorageRoster'),preview=el('ranchStoragePreview'),details=el('ranchStorageDetails');if(!roster||!preview||!details)return;const ids=state.storage.filter(Boolean);el('ranchStorageCount').textContent=`Storage ${ids.length}`;el('ranchActiveCount').textContent=`Ranch Active ${state.ranchActive.length}/${RANCH_ACTIVE_MAX}`;roster.innerHTML='';if(!ids.length){roster.innerHTML='<div class="manager-empty">Storage ว่าง</div>';preview.innerHTML='<div class="manager-empty">ยังไม่มีมอนสเตอร์ในคลัง</div>';details.innerHTML='';return;}if(!ids.includes(ranchStorageFocusId))ranchStorageFocusId=ids[0];ids.forEach(id=>{const i=getInst(id);if(!i)return;const card=document.createElement('button');card.type='button';card.className='manager-item';card.dataset.storageId=id;card.textContent=`${displayName(i)} • Lv.${i.level}`;card.classList.toggle('focused-monster',id===ranchStorageFocusId);card.onclick=()=>{ranchStorageFocusId=id;renderRanchStoragePage();};roster.appendChild(card);});const focused=getInst(ranchStorageFocusId);preview.innerHTML=focused?`<div class="focused-name">${displayName(focused)}</div><div>Lv.${focused.level} • HP ${fmt(focused.hp)}/${fmt(focused.maxHp)}</div>`:'';details.innerHTML=focused?`<div class="manager-empty">${state.party.includes(focused.instanceId)?'อยู่ Party • ฝากเข้าคลังได้':'อยู่ Storage • รับเข้า Party ได้เมื่อมีช่องว่าง'}</div><div class="storage-actions"><button type="button" data-storage-deposit>ฝากเข้าคลัง</button><button type="button" data-storage-withdraw>รับเข้า Party</button><button type="button" data-storage-ranch>${state.ranchActive.includes(focused.instanceId)?'เก็บจาก Ranch':'ปล่อย Ranch Active'}</button></div>`:'';details.querySelector('[data-storage-deposit]')?.addEventListener('click',()=>depositMonster(focused.instanceId));details.querySelector('[data-storage-withdraw]')?.addEventListener('click',()=>withdrawMonster(focused.instanceId));details.querySelector('[data-storage-ranch]')?.addEventListener('click',()=>toggleRanchActive(focused.instanceId));}
