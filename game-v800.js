...
import { advanceStatusLifecycle, clearStatusLifecycle, createStatusLifecycle } from './status-lifecycle.mjs';
...
function clearWilds(){
  ...
  for(const w of wilds){w.statusLifecycle=clearStatusLifecycle(w.statusLifecycle,'encounter_end').lifecycle;removeAndDispose(scene,w.mesh);removeWildLabel(w);}
  wilds.length=0;
...
function resetWild(w){if(w.dead)return;w.statusLifecycle=clearStatusLifecycle(w.statusLifecycle,'encounter_reset').lifecycle;w.hp=w.maxHp;w.state='wander';w.engaged=false;w.resetTimer=0;w.attackCd=0;w.mesh.position.copy(w.home);}
...
function defeatWild(w){
  if(w.dead)return;
  w.statusLifecycle=clearStatusLifecycle(w.statusLifecycle,'encounter_end').lifecycle;
  w.dead=true;
...
function updateWild(w,dt,canEngage=false){
  ...
  const statusTick=advanceStatusLifecycle(w.statusLifecycle,dt,{maxHp:w.maxHp});
  if(statusTick.ok){w.statusLifecycle=statusTick.lifecycle;if(statusTick.totalDamage>0){damageWild(w,Math.max(1,Math.round(statusTick.totalDamage)),{type:wildTypes(w)[0],eff:1,statusDamage:true});if(w.dead)return;}}
